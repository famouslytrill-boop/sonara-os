...
describe("Batch 14 agent, browser, voice and business-tool intake", () => {
  ...
  it("keeps security agents inside explicitly authorized lab boundaries", () => {
    ...
    for (const item of [byKey.agentic_bug_hunter, byKey.pentagi]) {
      ...
      assert.match(item.blockedUses.join(" "), /unauthorized|without authorization/i);
      assert.equal(item.enabledInProduction, false);
    }
    assert.match(byKey.pentagi.license, /SDK.*separate.*review/i);
  });

  it("uses anti-slop as a repository-owned quality-gate reference", () => {
...
describe("requested repository runtime surfaces", () => {
  it("publishes the governed public repository catalog", async () => {
    ...
    // disappear from the route while its own module tests still pass. Batch 18
    // adds 5 verified repository records and 10 non-repository references.
    assert.equal(response.body.repositoryCount, 145);
    assert.equal(response.body.verifiedCount, 141);
    assert.equal(response.body.blockedCount, 3);
    assert.equal(response.body.screenshotResearchCount, 135);
    assert.equal(response.body.unresolvedVisualLeadCount, 3);
    assert.equal(response.body.nonRepositoryReferenceCount, 78);
    assert.deepEqual(
  ...
  it("renders a public research page without executing external tools", async () => {
    ...
    assert.match(response.text, /135 additional developer, design, media, security, research, infrastructure, document, social, 3D, GPU, AI-workspace, and agent tools/);
    assert.match(response.text, /78 screenshot items are kept as hosted services, learning references, or unresolved non-repository leads/);
