...
// setting STRIPE_PRICE_WORKSPACE_MONTHLY to the wrong id would have advertised $29
...
describe("checkout will not sell at a price the page never showed", () => {
  ...
  it("allows a price that charges what the page advertises", async () => {
    global.fetch = async () => stripePriceResponse({ id: "price_ok", unit_amount: 2900, currency: "usd", active: true });
    const result = await build().assertPriceMatchesAdvertised("workspace_monthly", "price_ok");
    assert.equal(result.ok, true);
  });

  it("refuses a price that does not match what the page advertises", async () => {
    // Any different amount must be refused; this fixture uses $9.99 against $29.
    global.fetch = async () => stripePriceResponse({ id: "price_stale", unit_amount: 999, currency: "usd", active: true });
    const result = await build().assertPriceMatchesAdvertised("workspace_monthly", "price_stale");
    assert.equal(result.ok, false);
    assert.equal(result.code, "price_mismatch");
    assert.equal(result.charges, 999);
    assert.equal(result.advertised, 2900);
  });

  it("refuses an archived price", async () => {
    global.fetch = async () => stripePriceResponse({ id: "price_old", unit_amount: 2900, currency: "usd", active: false });
    const result = await build().assertPriceMatchesAdvertised("workspace_monthly", "price_old");
    assert.equal(result.ok, false);
  ...
  it("asks Stripe for the product, not just the price", async () => {
    ...
    global.fetch = async (url) => {
      ...
      return stripePriceResponse({ id: "price_ok", unit_amount: 2900, currency: "usd", active: true, product: { active: true } });
    };
    await build().assertPriceMatchesAdvertised("workspace_monthly", "price_ok");
  ...
  it("refuses a live price whose product is archived", async () => {
    global.fetch = async () =>
      stripePriceResponse({ id: "price_stale", unit_amount: 2900, currency: "usd", active: true, product: { active: false } });
    const result = await build().assertPriceMatchesAdvertised("workspace_monthly", "price_stale");
  ...
  it("treats a bare product id as unknown rather than archived", async () => {
    ...
    global.fetch = async () =>
      stripePriceResponse({ id: "price_ok", unit_amount: 2900, currency: "usd", active: true, product: "prod_something" });
    const result = await build().assertPriceMatchesAdvertised("workspace_monthly", "price_ok");
  ...
  it("refuses when Stripe will not say what the price is", async () => {
    ...
    const result = await build().assertPriceMatchesAdvertised("workspace_monthly", "price_missing");
    assert.equal(result.ok, false);
  ...
  it("refuses when the call to Stripe fails outright", async () => {
    ...
    const result = await build().assertPriceMatchesAdvertised("workspace_monthly", "price_x");
    assert.equal(result.ok, false);
    assert.equal(result.code, "price_unreadable");
  });
