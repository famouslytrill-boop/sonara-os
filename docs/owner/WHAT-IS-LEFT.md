...
across all 43 commands. The test count is deliberately not written here
...
**Some of it cannot be built as stated.** Of 227 reviewed repositories, 31 carry a
reciprocal licence (AGPL/GPL/OSL/SSPL), which triggers on network use and so
reaches a hosted product; 13 declare no licence at all — which is not a review
...
- **43** verification commands in the release chain
