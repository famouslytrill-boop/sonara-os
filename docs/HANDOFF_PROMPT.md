...
- Supabase over PostgREST for data. 93 migrations, 145 canonical tables. Every tenant-scoped table is filtered by `organization_id`; the service-role key never reaches a browser.
- 35 public routes, 17 customer routes, 29 admin routes.
- 195 test files run under mocha. `pnpm test` is the whole suite and takes about ten seconds.
...
155 external repositories have been reviewed and recorded in `data/open-source-tools.ts`. `docs/github-radar/GITHUB_RADAR_PRODUCT_INTEGRATION_MAP.md` says which product each one is for.
