...
`scripts/report-security-definer-exposure.mjs` reads the 128 migrations, finds
...
- **31 tables have RLS enabled with no explicit policy**, which closes
  ...
  Measured against the replay (all 128 migrations on an empty database, so this
