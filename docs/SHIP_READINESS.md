...
`scripts/report-security-definer-exposure.mjs` reads the 134 migrations, finds
...
- **31 tables have RLS enabled with no explicit policy**, which closes
  ...
  Measured against the replay (all 134 migrations on an empty database, so this
