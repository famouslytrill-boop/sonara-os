...
`scripts/report-security-definer-exposure.mjs` reads the 123 migrations, finds
...
- **31 tables have RLS enabled with no explicit policy**, which closes
  ...
  Measured against the replay (all 123 migrations on an empty database, so this
