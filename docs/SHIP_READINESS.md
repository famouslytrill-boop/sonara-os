...
`scripts/report-security-definer-exposure.mjs` reads the 127 migrations, finds
...
- **31 tables have RLS enabled with no explicit policy**, which closes
  ...
  Measured against the replay (all 127 migrations on an empty database, so this
