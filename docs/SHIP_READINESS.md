...
`scripts/report-security-definer-exposure.mjs` reads the 129 migrations, finds
...
`lib/`, **312 registered GET routes**, deployed as one serverless function.
...
- **31 tables have RLS enabled with no explicit policy**, which closes
  ...
  Measured against the replay (all 129 migrations on an empty database, so this
