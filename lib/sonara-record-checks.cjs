...
//
// Twenty-two checks: eleven for Business Builder, five for Creator Studio, six
// for Growth Studio. Several of the Creator and Growth ones enforce rules this
...
// How many days ago, or null because the row does not say.
//
// null and 0 are different answers and the difference matters: a row with no
// timestamp is a row nothing can be judged about, and returning 0 for it would
// read as "created just now" and quietly exclude it from every staleness check
// below. Every caller tests for null before comparing.
...
// A fortnight. See the note above the checks that use it -- it is a choice,
// not a measurement, and the sentences report the real age so the owner can
// disagree with it.
const STALE_DAYS = 14;
...
const CHECKS = Object.freeze([
  ...
  Object.freeze({
    id: "customer_invoices_overdue",
    product: "business_builder",
    severity: "money",
    table: "customer_invoices",
    columns: ["id", "invoice_number", "due_on", "total_cents", "status", "currency"],
    label: "Invoices your customers have not paid",
    // Only a sent invoice can be overdue. A draft nobody has seen is not late,
    // and counting it would put invented pressure on a number the owner reads
    // as real money owed.
    finds: (row) => {
      if (String(row.status || "").toLowerCase() !== "sent") return false;
      if (isBlank(row.due_on)) return false;
      const due = Date.parse(row.due_on);
      return Number.isFinite(due) && due < Date.now();
    },
    describe: (row) => `Invoice ${row.invoice_number || "(no number)"} for ${money(row.total_cents)} was due ${row.due_on} and is still marked sent.`,
    why: "This is money the business has already earned and has not been paid.",
    fixPath: "/business-builder/owner/receivables",
    fixLabel: "Open money owed to you"
  }),
  Object.freeze({
    id: "customer_invoices_sent_without_due_date",
    product: "business_builder",
    severity: "blocked",
    table: "customer_invoices",
    columns: ["id", "invoice_number", "due_on", "status"],
    label: "Sent invoices with no due date",
    // Without a due date the overdue check above cannot see this row at all.
    // An invoice that can never be chased is worse than a late one, because
    // nothing will ever raise its hand.
    finds: (row) => String(row.status || "").toLowerCase() === "sent" && isBlank(row.due_on),
    describe: (row) => `Invoice ${row.invoice_number || "(no number)"} was sent with no due date, so nothing will ever flag it as late.`,
    why: "An invoice with no due date cannot go overdue, so it drops out of every chase list silently.",
    fixPath: "/business-builder/owner/receivables",
    fixLabel: "Open money owed to you"
  }),
  Object.freeze({
  ...
  // Five checks that read one record against the calendar, added because the
  // eleven above answered "is this row filled in?" and none of them answered
  // "is this row stuck?". A quote sent in March and never answered is complete,
  // valid, and worth nothing, and every check written before these looked
  // straight past it.
  //
  // STALE_DAYS is the one number they share. Fourteen days is not a discovered
  // constant and is not presented as one: it is a fortnight, chosen because it
  // is long enough that a business that simply had a busy week is not nagged,
  // and short enough that the money is still collectable. The sentences say the
  // real age of the record, so an owner can disagree with the threshold and
  // still act on the fact.
  Object.freeze({
    id: "customer_invoices_draft_and_aging",
    product: "business_builder",
    severity: "money",
    table: "customer_invoices",
    columns: ["id", "invoice_number", "total_cents", "status", "created_at"],
    label: "Invoices written and never sent",
    // customer_invoices_overdue deliberately ignores drafts, because a draft
    // cannot be late. That leaves a gap this fills: work that was done, priced,
    // written up, and never actually asked for. It is not late -- nobody has
    // ever been asked to pay it, which is worse, and no other check can see it.
    finds: (row) => {
      if (String(row.status || "").toLowerCase() !== "draft") return false;
      const age = ageInDays(row.created_at);
      return age !== null && age >= STALE_DAYS;
    },
    describe: (row) => `Invoice ${row.invoice_number || "(no number)"} for ${money(row.total_cents)} has been a draft for ${Math.floor(ageInDays(row.created_at))} days and has never been sent.`,
    why: "The work is done and the customer has never been asked to pay for it.",
    fixPath: "/business-builder/owner/invoices",
    fixLabel: "Open invoices"
  }),
  Object.freeze({
    id: "quotes_sent_without_answer",
    product: "business_builder",
    severity: "money",
    table: "quotes",
    columns: ["id", "title", "amount_cents", "status", "updated_at"],
    label: "Quotes nobody has answered",
    // Measured from updated_at rather than created_at: sending a quote writes
    // the status, so updated_at is when the customer last heard from you.
    // Reading created_at would call a quote drafted in January and sent
    // yesterday six months stale.
    finds: (row) => {
      if (String(row.status || "").toLowerCase() !== "sent") return false;
      const age = ageInDays(row.updated_at);
      return age !== null && age >= STALE_DAYS;
    },
    describe: (row) => `${row.title || "A quote"} for ${money(row.amount_cents)} was sent ${Math.floor(ageInDays(row.updated_at))} days ago and is still waiting on an answer.`,
    why: "A quote with no answer is not a no. Most are won by whoever follows up.",
    fixPath: "/business-builder/owner/quotes",
    fixLabel: "Open quotes"
  }),
  Object.freeze({
    id: "quotes_sent_without_amount",
    product: "business_builder",
    severity: "blocked",
    table: "quotes",
    columns: ["id", "title", "amount_cents", "status"],
    label: "Quotes sent with no price",
    // isMissingNumber treats 0 as missing, which is right here: a quote for
    // nothing is a quote somebody has not finished, not a free job. A genuinely
    // free job is recorded as an accepted quote with a note, not as a price.
    finds: (row) => {
      const status = String(row.status || "").toLowerCase();
      if (status !== "sent" && status !== "accepted") return false;
      return isMissingNumber(row.amount_cents);
    },
    describe: (row) => `${row.title || "A quote"} is marked ${row.status} and has no price on it.`,
    why: "Nothing can be invoiced from it, and the customer has no figure to agree to.",
    fixPath: "/business-builder/owner/quotes",
    fixLabel: "Open quotes"
  }),
  Object.freeze({
    id: "bookings_past_and_still_open",
    product: "business_builder",
    severity: "blocked",
    table: "business_bookings",
    columns: ["id", "customer_name", "starts_at", "ends_at", "status"],
    label: "Appointments that happened and were never closed off",
    // The status vocabulary is a CHECK constraint on the table
    // (requested, confirmed, completed, cancelled, no_show, archived), so these
    // two names are the only open ones and are not guesses. ends_at is used
    // where it exists because a booking is not over when it starts.
    finds: (row) => {
      const status = String(row.status || "").toLowerCase();
      if (status !== "requested" && status !== "confirmed") return false;
      const finished = Date.parse(row.ends_at || row.starts_at || "");
      return Number.isFinite(finished) && finished < Date.now();
    },
    describe: (row) => `${row.customer_name || "An appointment"} on ${String(row.starts_at || "").slice(0, 10) || "an unrecorded date"} is still marked ${row.status}.`,
    why: "Until it is marked completed or no-show it will not be invoiced and will not count in anything you measure.",
    fixPath: "/business-builder/owner/bookings",
    fixLabel: "Open bookings"
  }),
  Object.freeze({
    id: "customers_without_contact",
    product: "business_builder",
    severity: "blocked",
    table: "customers",
    columns: ["id", "name", "email", "phone", "status"],
    label: "Customers with no way to reach them",
    // leads_without_contact covers growth_leads and bookings_without_contact
    // covers a single appointment. Neither looks at the customer list itself,
    // which is the record every quote, invoice and reminder is addressed from.
    finds: (row) => String(row.status || "").toLowerCase() !== "archived" && isBlank(row.email) && isBlank(row.phone),
    describe: (row) => `${row.name || "A customer"} has neither an email address nor a phone number.`,
    why: "Every quote, invoice and reminder is sent to one of those two. Without either, this customer cannot be contacted at all.",
    fixPath: "/business-builder/owner/customers",
    fixLabel: "Open customers"
  }),
