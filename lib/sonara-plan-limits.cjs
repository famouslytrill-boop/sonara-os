...
// Annual plans get their twin's allowance. Both tables below fall back to the
// free allowance for a key they do not list, so an unlisted annual plan would
// silently sell a yearly subscription with the free tier's limits.
const { ANNUAL_TWINS } = require("./sonara-stripe-plans.cjs");

/** Give every annual plan the allowance recorded for the monthly plan it mirrors. */
function withAnnualAllowances(allowances) {
  const expanded = { ...allowances };
  for (const [annual, monthly] of Object.entries(ANNUAL_TWINS)) {
    if (Object.prototype.hasOwnProperty.call(allowances, monthly)) expanded[annual] = allowances[monthly];
  }
  return expanded;
}
...
const INCLUDED_LOCATIONS = Object.freeze(withAnnualAllowances({
  ...
  // workspace.
  free: 1,
  // The one-time Business Builder setup purchase remains a quoted entitlement.
...
const INCLUDED_SCROLL_SITES = Object.freeze(withAnnualAllowances({
  free: 1,
  business_builder_one_time: 1,
  workspace_monthly: 1,
  all_three_monthly: null,
