...
function createReadiness(deps = {}) {
  const { getEnv, isPlaceholderValue, isEmailLike, isPlaceholderEmail, splitList, STRIPE_PLANS, getLegalPagesStatus } = deps;
  ...
  function getReadiness() {
    ...
    // A price variable for a plan the page deliberately does not show yet is
    // deferred, not missing.
    //
    // Every unset plan price landed in `missing.stripe` until 8 September 2026,
    // which was right while every plan was meant to be buyable. The three annual
    // plans are not: they carry `hiddenUntilBuyable`, so `offeredPlanKeys` keeps
    // them off the pricing page entirely until their Stripe prices exist, and
    // docs/owner/PRICE-CUTOVER-RUNBOOK.md names staging the monthly ladder first
    // as a supported pathway.
    //
    // A superseded plan becomes the same kind of deferred configuration once
    // its replacement is actually buyable. Production completed that cutover
    // in September 2026: the breadth plans were configured and the old
    // starter/core/pro prices were intentionally absent. Calling those retired
    // checkout variables "missing" made readiness claim a broken payment system
    // after the replacement system had opened successfully.
    //
    // Deferred plan variables remain visible until their runtime definitions are
    // removed. Google sign-in is no longer part of this bucket: its live provider
    // state is verified directly by the auth route and /api/readiness.
    for (const [plan, planStatus] of Object.entries(checkoutPlans)) {
      if (!planStatus.env || planStatus.reason !== "missing") continue;
      const config = STRIPE_PLANS[plan];
      const successorEnabled = Boolean(
        config?.supersededBy && checkoutPlans[config.supersededBy]?.checkout === "enabled"
      );
      if (config?.hiddenUntilBuyable || successorEnabled) stripeDeferred.push(planStatus.env);
      else stripeMissing.push(planStatus.env);
    }
    const services = {
      ...
      googleOAuth: "setup_required",
      adminProtection: adminProtectionStatus.status,
      // See lib/sonara-legal-position.cjs. This was the literal
      // "review_required" -- an item on a setup list that no change to this
      // code could ever close, because a qualified legal review is a decision
      // about engaging counsel. The position is a disclaimer, and a disclaimer
      // is finished. It is derived so that deleting the disclaimer from the
      // pages changes what this says.
      //
      // The fallback is not the finished state. A caller that forgets to inject
      // the reader gets "unknown" rather than the good answer, because a status
      // that reports success when nobody computed it is the failure this whole
      // change is about.
      legalPages: typeof getLegalPagesStatus === "function" ? getLegalPagesStatus() : "unknown",
      ...
      googleSignIn: "setup_required",
      founderAccess: founderAccessStatus.status
    };
    // What `invalid` holds, computed before the payload so `ok` can be derived
    // from it rather than asserted alongside it.
    const invalid = {
      supabase: supabaseStatus.invalid,
      stripe: getInvalidStripeEnvStatuses(),
      resend: resendStatus.invalid,
      founderAccess: founderAccessStatus.invalid,
      adminProtection: adminProtectionStatus.invalid
    };
    const invalidCount = Object.values(invalid).reduce(
      (total, list) => total + (Array.isArray(list) ? list.length : 0),
      0
    );

    return {
      // `ok` was the literal `true` for the life of this function: a field named
      // `ok` that no input could ever make false.
      //
      // On 9 September 2026 production served `ok: true` and
      // `services.stripe: "configured"` while `invalid.stripe` named all three
      // annual price variables with `invalid_prefix` -- they had been set to
      // something that is not a `price_` id, so the yearly plans could not be
      // sold. The smoke check read `invalid` and failed correctly; a person
      // reading the summary saw green. Both were looking at the same response.
      //
      // Derived from `invalid` only, and deliberately not from `missing` or
      // `deferred`. Those are choices: an optional capability nobody has
      // configured is the product working as intended, and folding them in would
      // make `ok` false forever and therefore meaningless again. `invalid` is
      // different in kind -- it means a variable IS set and is set to something
      // that cannot work, which is never a resting state somebody chose.
      ok: invalidCount === 0,
      ...
      services,
      checkoutPlans,
      missing: { supabase: supabaseStatus.missing, stripe: stripeMissing, resend: resendStatus.missing, googleOAuth: ["Supabase Google provider"], adminProtection: adminProtectionStatus.missing },
      deferred: { stripe: stripeDeferred },
      invalid
    };
  }
  ...
  function getAdminEnvReadiness() {
    ...
    return [
      ...
      readinessItem("STRIPE_SECRET_KEY", "STRIPE_SECRET_KEY", getStripeSecretStatus().status, "Stripe API key should start with sk_live_, sk_test_, rk_live_, or rk_test_."),
  ...
  function getStripeSecretStatus() {
    ...
    // Stripe restricted keys are valid API keys too. Least-privilege keys are
    // already used by the release verifier, and a production restricted key
    // may be granted only the operations the runtime needs. Prefix validation
    // can reject malformed values; actual permissions are proven by the calls
    // that use the key, not by requiring the unrestricted `sk_` shape.
    if (!startsWithAny(value, ["sk_live_", "sk_test_", "rk_live_", "rk_test_"])) return { status: "invalid_prefix" };
    return { status: "configured" };
  }
  ...
  function getStripePlanPriceStatus(plan) {
    const config = STRIPE_PLANS[plan];
    // Quoted work is not sold through checkout, so there is no price to
    // configure and nothing to report as missing. This has to come before the
    // no-env branch below, which was written for the free plan and would
    // otherwise report a quoted plan's checkout as "enabled".
    if (config?.quoted) return { status: "quoted", checkout: "quoted", env: undefined, reason: "quoted" };
  ...
  function isStripePriceId(value) {
    ...
    // Stripe price IDs are opaque, but the API format is a `price_` prefix
    // followed by an identifier. Reject display values and other Stripe
    // objects even when somebody prepends `price_` accidentally.
    return /^price_[A-Za-z0-9]+$/.test(candidate) && !isPlaceholderValue(candidate);
  }
