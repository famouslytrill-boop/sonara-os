...
function createReadiness(deps = {}) {
  ...
  function getReadiness() {
    ...
    //
    // Reporting them as missing made that pathway fail CI: scripts/smoke-live-routes.mjs
    // fails when `missing.stripe` is non-empty, so from the moment the owner set
    // the three monthly variables, every pull request went red on production
    // connectivity until three prices nobody had promised to create existed. The
    // deployment was fine; the classification was wrong.
    //
    ...
    for (const [plan, planStatus] of Object.entries(checkoutPlans)) {
      ...
      if (STRIPE_PLANS[plan] && STRIPE_PLANS[plan].hiddenUntilBuyable) stripeDeferred.push(planStatus.env);
