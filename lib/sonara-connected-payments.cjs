...
async function readAccount(deps, organizationId, fetchImpl = fetch) {
  ...
  // Named rather than `select=*`. Every column here has a reader:
  // `stripe_account_id` is what Stripe is asked about, and the four cached
  // answers are what the page falls back to when Stripe cannot be reached.
  // A star select fetches thirteen columns and hides which of them anything
  // needs, which is what report-unused-selected-columns cannot see into.
  const url =
    `${deps.supabaseUrl}/rest/v1/business_payment_accounts` +
    `?organization_id=eq.${encodeURIComponent(organizationId)}` +
    "&disconnected_at=is.null" +
    "&select=id,stripe_account_id,charges_enabled,payouts_enabled,details_submitted,state_checked_at" +
    "&limit=1";
  let response;
  try {
...
async function canAcceptPayments(deps, organizationId, fetchImpl = fetch) {
  ...
  return {
    ...
    requirementsDue: live.requirementsDue,
    // What Stripe just said, handed back so a caller that is rendering a page
    // can write it onto the row. Not cached here: this function is on the money
    // path, and a write per payment check is a cost per payment.
    state: live
  };
}
...
/**
 * Write Stripe's answers onto the row.
 *
 * **This had no caller until 3 September 2026.** It was written for a list that
 * was never built, so the four columns it fills were null on every row in
 * production, `readAccount` fetched them with `select=*`, and nothing read
 * them. A function that is exported, looks finished, and is invoked by nothing
 * is the shape this repository keeps finding.
 *
 * What calls it now is the connected-payments page, after Stripe has answered.
 * Not `canAcceptPayments`, which is on the money path -- a write per payment
 * check is a cost per payment, and money deliberately re-reads Stripe anyway.
 *
 * What the cache is for: when Stripe cannot be reached, the page can say what
 * was true when it was last asked, and when that was, instead of only "could
 * not check". Never for deciding whether to take a payment -- a stale
 * `charges_enabled: true` renders a pay button on an account that cannot take
 * money, which is the note at the top of this file.
 *
