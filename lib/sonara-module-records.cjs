...
function createModuleRecords(deps = {}) {
  ...
  async function safeReadOrganizationScopedRecords(organizationId, productKey) {
    ...
    const response = await fetch(`${config.url}/rest/v1/module_outputs?select=id,module_key,created_at,output_payload,share_token,shared_at&organization_id=eq.${encodeURIComponent(organizationId)}&product_key=eq.${encodeURIComponent(productKey)}&order=created_at.desc&limit=20`, {
