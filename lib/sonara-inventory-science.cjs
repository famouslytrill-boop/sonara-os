...
// ---------------------------------------------------------------------------
// Store and retail operating arithmetic.
// ---------------------------------------------------------------------------
//
// These calculations deliberately consume reconciled operational inputs instead
// of querying a provider or guessing missing state. They are small enough to
// audit, cheap enough to run everywhere, and important enough that a model should
// never be the calculator of record.
...
function inventoryAvailability({
  ...
  const onHand = Number(onHandUnits);
  const allocated = Number(allocatedUnits);
  const safety = Number(safetyStockUnits);
  const inbound = Number(confirmedInboundUnits);
  ...
  for (const [name, value] of [
    ...
    if (!Number.isFinite(value) || value < 0) {
      return {
        ok: false,
        code: `${name}_invalid`,
        message: `${name.replaceAll("_", " ")} units must be a finite number at or above zero.`
      };
    }
  }

  const rawAvailableUnits = onHand - allocated - safety;
  const availableToSellUnits = Math.max(0, rawAvailableUnits);
  return {
    ok: true,
    onHandUnits: onHand,
    allocatedUnits: allocated,
    safetyStockUnits: safety,
    confirmedInboundUnits: inbound,
    rawAvailableUnits,
    availableToSellUnits,
    shortfallUnits: Math.max(0, -rawAvailableUnits),
    projectedAvailableUnits: Math.max(0, rawAvailableUnits + inbound),
    countsInboundAsAvailableNow: false
  };
}

function sellThroughRate({ unitsSold, startingUnits, receivedUnits = 0 }) {
  const sold = Number(unitsSold);
  const starting = Number(startingUnits);
  const received = Number(receivedUnits);
  for (const [name, value] of [["units_sold", sold], ["starting_units", starting], ["received_units", received]]) {
    if (!Number.isFinite(value) || value < 0) {
      return { ok: false, code: `${name}_invalid`, message: `${name.replaceAll("_", " ")} must be a finite number at or above zero.` };
    }
  }

  const unitsAvailableDuringPeriod = starting + received;
  if (unitsAvailableDuringPeriod <= 0) {
    return {
      ok: false,
      code: "sell_through_denominator_required",
      message: "Sell-through needs starting stock or received stock for the period."
    };
  }
  if (sold > unitsAvailableDuringPeriod) {
    return {
      ok: false,
      code: "sales_exceed_recorded_available_units",
      message: "Units sold exceed starting plus received units. Reconcile transfers, returns, adjustments, or the period before calculating sell-through."
    };
  }

  const rate = sold / unitsAvailableDuringPeriod;
  return {
    ok: true,
    unitsSold: sold,
    unitsAvailableDuringPeriod,
    rate,
    percent: rate * 100
  };
}
...
function inventoryTurnover({
  ...
  beginningInventoryCostCents,
  endingInventoryCostCents
}) {
  const cogs = Number(costOfGoodsSoldCents);
  const beginning = Number(beginningInventoryCostCents);
  const ending = Number(endingInventoryCostCents);
  for (const [name, value] of [["cogs", cogs], ["beginning_inventory", beginning], ["ending_inventory", ending]]) {
    if (!Number.isFinite(value) || value < 0) {
      return { ok: false, code: `${name}_invalid`, message: `${name.replaceAll("_", " ")} cost must be a finite number at or above zero.` };
    }
  }

  const averageInventoryCostCents = (beginning + ending) / 2;
  if (averageInventoryCostCents <= 0) {
    return {
      ok: false,
      code: "average_inventory_required",
      message: "Inventory turnover needs a positive average inventory cost."
    };
  }
  ...
  return {
    ...
    averageInventoryCostCents,
    turns: cogs / averageInventoryCostCents
  };
}

function daysInventoryOnHand({
  costOfGoodsSoldCents,
  averageInventoryCostCents,
  periodDays
}) {
  const cogs = Number(costOfGoodsSoldCents);
  const average = Number(averageInventoryCostCents);
  const days = Number(periodDays);

  if (!Number.isFinite(cogs) || cogs <= 0) {
    return { ok: false, code: "cogs_required", message: "Days of inventory on hand needs positive cost of goods sold for the period." };
  }
  if (!Number.isFinite(average) || average < 0) {
    return { ok: false, code: "average_inventory_invalid", message: "Average inventory cost must be a finite number at or above zero." };
  }
  if (!Number.isFinite(days) || days <= 0) {
    return { ok: false, code: "period_days_required", message: "Days of inventory on hand needs a positive period length in days." };
  }

  return {
    ok: true,
    averageInventoryCostCents: average,
    costOfGoodsSoldCents: cogs,
    periodDays: days,
    days: (average / cogs) * days
  };
}
...
function grossMarginReturnOnInventory({
  ...
  const margin = Number(grossMarginCents);
  const average = Number(averageInventoryCostCents);

  if (!Number.isFinite(margin)) {
    return { ok: false, code: "gross_margin_required", message: "GMROI needs a reconciled gross-margin amount." };
  }
  if (!Number.isFinite(average) || average <= 0) {
    return { ok: false, code: "average_inventory_required", message: "GMROI needs a positive average inventory cost." };
  }
  ...
  return {
    ...
    averageInventoryCostCents: average,
    ratio: margin / average
  };
}

function inventoryVariance({ bookUnits, countedUnits }) {
  const book = Number(bookUnits);
  const counted = Number(countedUnits);
  for (const [name, value] of [["book_units", book], ["counted_units", counted]]) {
    if (!Number.isFinite(value) || value < 0) {
      return { ok: false, code: `${name}_invalid`, message: `${name.replaceAll("_", " ")} must be a finite number at or above zero.` };
    }
  }

  const varianceUnits = counted - book;
  const shrinkUnits = book - counted;
  return {
    ok: true,
    bookUnits: book,
    ...
    varianceUnits,
    shrinkUnits,
    varianceRate: book > 0 ? varianceUnits / book : null,
    shrinkRate: book > 0 ? shrinkUnits / book : null,
    percentageUnavailableBecauseBookIsZero: book === 0
  };
}

function weeksOfSupply({ availableUnits, averageWeeklyDemandUnits }) {
  const available = Number(availableUnits);
  const demand = Number(averageWeeklyDemandUnits);
  if (!Number.isFinite(available) || available < 0) {
    return { ok: false, code: "available_units_invalid", message: "Available units must be a finite number at or above zero." };
  }
  if (!Number.isFinite(demand) || demand <= 0) {
    return { ok: false, code: "weekly_demand_required", message: "Weeks of supply needs positive average weekly demand." };
  }

  return {
    ok: true,
    availableUnits: available,
    averageWeeklyDemandUnits: demand,
    weeks: available / demand
  };
}
