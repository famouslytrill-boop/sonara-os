...
// Fail-open has to be loud, and it was loud in exactly one place out of four.
//
...
// abuse surface -- degraded to no limit at all and said nothing about it.
//
...
function defaultDegradedReport({ name, error }) {
  console.error(`[rate-limit] ${name} degraded to fail-open: ${redactError(error, { includeStack: false })}`);
}

function createRateLimiter({ name, windowSeconds, maxAttempts, scopes = ["ip"], subjectFrom, getSupabaseServerConfig, onDegraded = defaultDegradedReport, renderDenied }) {
