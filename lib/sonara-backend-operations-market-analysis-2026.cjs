...
function executionFabricDecision(input = {}) {
  const eventsPerSecond = nonNegative(input.eventsPerSecond ?? 0, "eventsPerSecond");
  const workflowDurationMinutes = nonNegative(input.workflowDurationMinutes ?? 0, "workflowDurationMinutes");
  const humanWaits = Boolean(input.humanWaits);
  const crossServiceSteps = Boolean(input.crossServiceSteps);
  const requiresReplay = Boolean(input.requiresReplay);
  const databaseAdjacent = Boolean(input.databaseAdjacent);
  const offlineEdge = Boolean(input.offlineEdge);
  const subsecondCoordination = Boolean(input.subsecondCoordination);

  if (offlineEdge) {
    return Object.freeze({ mode: "offline_command_log", adoption: "existing_pattern", reason: "edge_connectivity_not_guaranteed" });
  }
  if (subsecondCoordination) {
    return Object.freeze({ mode: "realtime_coordinator", adoption: "evaluate_only", reason: "serialized_low_latency_state_required" });
  }
  if (humanWaits || crossServiceSteps || workflowDurationMinutes >= 5) {
    return Object.freeze({ mode: "durable_workflow", adoption: "evaluate_isolated", reason: "long_lived_or_interactive_workflow" });
  }
  if (requiresReplay && eventsPerSecond >= 1000) {
    return Object.freeze({ mode: "stream_fabric", adoption: "evaluate_only", reason: "measured_high_throughput_replay_requirement" });
  }
  if (databaseAdjacent) {
    return Object.freeze({ mode: "postgres_outbox_inbox", adoption: "existing_pattern", reason: "transactional_adjacency" });
  }
  return Object.freeze({ mode: "persisted_job_queue", adoption: "existing_pattern", reason: "bounded_background_work" });
}

function autonomicRepairDecision(input = {}) {
  const evidenceFreshness = unit(input.evidenceFreshness, "evidenceFreshness");
  const blastRadius = unit(input.blastRadius, "blastRadius");
  const rollbackConfidence = unit(input.rollbackConfidence, "rollbackConfidence");
  const dataLossRisk = unit(input.dataLossRisk ?? 0, "dataLossRisk");
  const deterministic = Boolean(input.deterministic);
  const reversible = Boolean(input.reversible);
  const abortable = Boolean(input.abortable);
  const tenantScoped = Boolean(input.tenantScoped);
  const authoritySensitive = Boolean(input.authoritySensitive);
  const changesCode = Boolean(input.changesCode);
  const changesSchema = Boolean(input.changesSchema);
  ...
  if (authoritySensitive) return Object.freeze({ automate: false, mode: "human_approval_required", reason: "authority_sensitive" });
  if (changesCode || changesSchema) return Object.freeze({ automate: false, mode: "branch_only", reason: "source_or_schema_change" });
  if (dataLossRisk > 0.01) return Object.freeze({ automate: false, mode: "human_approval_required", reason: "data_loss_risk" });
  if (evidenceFreshness < 0.9) return Object.freeze({ automate: false, mode: "observe_only", reason: "evidence_not_fresh_enough" });
  if (!deterministic || !reversible || !abortable) return Object.freeze({ automate: false, mode: "observe_only", reason: "repair_not_proven_controllable" });
  if (rollbackConfidence < 0.9) return Object.freeze({ automate: false, mode: "approval_required", reason: "rollback_confidence_too_low" });
  if (blastRadius > 0.05) return Object.freeze({ automate: false, mode: "approval_required", reason: "blast_radius_too_large" });

  return Object.freeze({ automate: true, mode: "bounded_reconciliation", reason: "fresh_reversible_abortable_low_risk_repair" });
}
...
function getBackendOperationsMarketAnalysis() {
  return {
    ...
    referenceSystemCount: BACKEND_REFERENCE_SYSTEMS_2026.length,
    ...
    referenceSystems: BACKEND_REFERENCE_SYSTEMS_2026.map((item) => ({ ...item })),
    formulas: {
      ...
      autonomicRepair: "tenant_scoped && fresh_evidence>=0.9 && deterministic && reversible && abortable && rollback_confidence>=0.9 && blast_radius<=0.05 && data_loss_risk<=0.01 && !authority_sensitive",
      executionFabric: "edge -> realtime -> durable-workflow -> high-throughput replay -> postgres outbox/inbox -> persisted job queue"
    },
    architectureDecision: {
      ...
      executionFabric: "Default to existing Postgres/outbox/inbox and persisted jobs. Evaluate DBOS, Temporal, Restate or a stream fabric only behind an isolated adapter when measured duration, interaction, replay or throughput requirements exceed the existing path.",
      protocols: "Use interoperable specifications such as CloudEvents, OpenFeature and MCP as boundaries, not as authority. Tenant identity, authorization, billing, secrets and audit remain SONARA-owned."
    },
    guardrails: [
      ...
      "App-store notifications and agent protocols are evidence/transport inputs only; canonical entitlement and tool authority remain server-side SONARA state.",
      "Reference-system catalog entries are not installation, dependency, provider, migration or production enablement approvals."
    ]
  };
}
