...
const CAPABILITIES = Object.freeze([
  capability({
    ...
    existing: ["agent authority", "entity agent schema", "agent action logs", "workflow runs", "provider gateway"],
    next: ["durable action execution", "approval-linked action runs", "bounded tool registry", "cost ceilings"],
    data: ["entity_agents", "entity_agent_runs", "entity_agent_tool_registry", "entity_action_runs", "entity_action_approvals", "agent_action_logs"],
  ...
  capability({
    ...
    name: "Interactive Media and Design Studio",
    form: FORMS.ADD_ON,
    status: STATUS.IMPLEMENT_NEXT,
    targets: ["creator_studio", "growth_studio", "business_builder"],
    value: "Unify images, video, audio, brand assets, layout, motion, interactive components, and export around editable project state.",
    existing: ["creator assets", "generation lifecycle", "scroll sites", "video treatments", "brand governance"],
    next: ["design document contract", "component tree", "design tokens", "timeline", "responsive rules", "approval/export states"],
    data: ["creator_assets", "creator_video_treatments", "scroll_sites", "generation_jobs", "generation_artifacts"],
    workflow: ["brief", "compose", "generate/import", "edit", "review", "approve", "export/publish"],
    ux: ["business-oriented canvas", "inspector", "asset rail", "timeline", "responsive preview"]
  }),
  capability({
