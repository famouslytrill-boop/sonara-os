...
function getConfig(getSupabaseServerConfig) {
  ...
  if (!config?.ok || !url || !serviceRoleKey || !isTrustedSupabaseOrigin(url)) return { ok: false };
  return { ok: true, url, serviceRoleKey };
}

// This transport carries a service-role credential, so its destination is part
// of the authorization boundary. The first PGMQ canary is intentionally limited
// to hosted Supabase project origins; a typo, injected URL or future config
// regression must fail before fetch receives the credential.
function isTrustedSupabaseOrigin(value) {
  try {
