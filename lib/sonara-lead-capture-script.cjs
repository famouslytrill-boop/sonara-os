...
// What a stranger is allowed to write into somebody else's records.
//
// `cleanText` deliberately does not bound length, because most of its callers
// read the OWNER's own profile configuration -- industries, regions,
// disqualifiers -- and silently truncating what a business typed about itself
// would be its own defect. The bound belongs where untrusted input enters, and
// that is `recordAnswer`: every answer from `/chat/:slug` funnels through it,
// and `/chat/:slug` is one of the two endpoints in this application a person
// with no account can write through.
//
// Until 4 September 2026 nothing bounded it. `express.urlencoded({ limit: "1mb" })`
// and the chat rate limiter kept it from being unbounded, so this was never a
// way to fill a database -- but a visitor could still put a megabyte in the
// `name` field, and that lead then renders on the owner's list page. The owner
// is the one who suffers, and they did not do anything.
//
// The numbers are the public booking page's, which has clamped since it was
// written. Two public write paths disagreeing about how long a phone number can
// be is the kind of difference that turns into a bug report nobody can
// reproduce.
const VISITOR_LIMITS = Object.freeze({ name: 120, email: 320, phone: 40, free: 1000 });

/**
 * Bound a visitor's answer without changing what an empty one means.
 *
 * Truncating is right for text somebody will read -- a name, a free-text
 * answer. The tail is lost and nothing else changes.
 */
function visitorText(value, limit) {
  const text = cleanText(value);
  return text === null ? null : text.slice(0, limit);
}

/**
 * The same bound for an identifier, which must be **refused rather than cut**.
 *
 * This is the half the first version of this fix got wrong. An email address or
 * a phone number is not display text: truncating one does not shorten it, it
 * makes it point somewhere else. `someone@example.com.attacker.tld` cut at the
 * wrong byte is `someone@example.com` -- a different, real person's address,
 * stored as though the visitor had typed it, and then emailed by whatever the
 * business does with a lead.
 *
 * Landing that exactly is fiddly, and that is not the standard worth applying
 * to somebody else's contact details. An over-long identifier is refused, and
 * the visitor is told, because "we could not accept that" is always better than
 * quietly recording a value they did not give.
 */
function visitorIdentifier(value, limit) {
  const text = cleanText(value);
...
function recordAnswer(profile = {}, key, raw) {
  ...
  if (question.kind === "choice") {
    const text = visitorText(typeof raw === "object" && raw !== null ? raw.value : raw, VISITOR_LIMITS.free);
    if (text === null) return { ok: false, code: "blank", message: "Pick one, or say a bit more." };
    if (text === OTHER_VALUE) {
      const other = visitorText(typeof raw === "object" && raw !== null ? raw.other : null, VISITOR_LIMITS.free);
  ...
  if (question.kind === "contact") {
    ...
    const name = visitorText(source.name, VISITOR_LIMITS.name);
    // Identifiers are refused when over-long rather than cut down. See
    // visitorIdentifier: a truncated address is a different address.
    const emailField = visitorIdentifier(source.email, VISITOR_LIMITS.email);
    if (!emailField.ok) {
      return { ok: false, code: "email_too_long", message: "That email address is too long to record." };
    }
    const phoneField = visitorIdentifier(source.phone, VISITOR_LIMITS.phone);
    if (!phoneField.ok) {
      return { ok: false, code: "phone_too_long", message: "That phone number is too long to record." };
    }
...
module.exports = {
  questionsFor,
  recordAnswer,
  // Exported so the test asserts the real numbers rather than a second copy.
  VISITOR_LIMITS,
  nextStep,
  transcriptActivity,
  scorableAnswers,
