...
function renderSetupStateCard(state, escapeHtml = escape) {
  ...
  return `<article class="card sonara-setup-state" role="status">
    ...
    <ul class="sonara-checklist">
      <li><strong>Next step:</strong> ${escapeHtml(state.primaryLabel)}.</li>
      <li><strong>Your data:</strong> ${escapeHtml(state.dataSafety)}</li>
      <li><strong>While you wait:</strong> ${escapeHtml(state.continuity)}</li>
    </ul>
  </article>`;
}
