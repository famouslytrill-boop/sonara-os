...
const OWNER_RECORD_PAGES = Object.freeze([
  ...
  },
  {
    ...
    derivedCard: (day, childRows, ui) => {
      ...
      return ui.card(
        "What this day made",
        `Net sales ${money(netSales)}, food cost ${money(foodCost)}${share}, leaving ${money(netSales - foodCost)} before labour. ` +
          "Labour is not in this figure: it needs hours and a pay rate, and those are recorded against your staff rather than against the day."
      );
    }
  },
  {
    ...
    body: "A day at a time: what you took, what it cost you and what was left. The food cost side is worked out on each day under Daily sales; this page waits on labour, which needs the hours your staff record and the rates you set against them.",
    empty: "Nothing here yet. Each day's food cost is shown on the day itself under Daily sales; combining that with labour into one figure per day is not built.",
    columns: [
      { label: "Day", value: (row) => day(row.business_date) },
