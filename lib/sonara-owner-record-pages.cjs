...
const workOrderLifecycle = require("./sonara-work-order-lifecycle.cjs");
...
const OWNER_RECORD_PAGES = Object.freeze([
  ...
  },
  {
    ...
    form: {
      ...
      fields: [
        { name: "employee_id", label: "Who is starting", type: "reference", from: "staff", required: true },
        { name: "location_id", label: "Where", type: "reference", from: "locations" },
        { name: "work_order_id", label: "Work order", type: "reference", from: "workOrders", hint: "Optional. Link these hours to the job they belong to." },
...
const REFERENCE_SOURCES = Object.freeze({
  ...
  workOrders: { table: "business_work_orders", label: (row) => row.work_order_number || row.title || row.id },
  bookings: { table: "business_bookings", label: (row) => row.customer_name || row.starts_at || row.id },
  vehicles: { table: "vehicle_records", label: (row) => row.registration_number || row.plate_number || row.vehicle_type || row.id },
  routeSessions: { table: "route_tracking_sessions", label: (row) => [row.session_type, row.started_at ? String(row.started_at).slice(0, 16).replace("T", " ") : ""].filter(Boolean).join(" — ") || row.id },
  productVariants: {
...
const OPERATIONS_RECORD_PAGES = Object.freeze([
  {
    path: "/business-builder/owner/work-orders",
    select: "agreed_amount_cents,currency,id,priority,scheduled_start_at,status,title,work_order_number",
    api: "/api/business/work-orders",
    table: "business_work_orders",
    lifecycleManaged: true,
    title: "Work orders",
    body: "Jobs from agreement through scheduled work, completion, invoicing and closeout.",
    empty: "You have not started a work order yet. Add a job below or start one from an accepted quote.",
    columns: [
      { label: "Job", value: (row) => text(row.work_order_number || row.title, "Untitled") },
      { label: "Starts", value: (row) => when(row.scheduled_start_at) },
      { label: "Priority", value: (row) => words(row.priority) },
      { label: "Agreed", value: (row) => money(row.agreed_amount_cents, row.currency) },
      { label: "Status", value: (row) => words(row.status) }
    ],
    form: {
      legend: "Add a work order",
      fields: [
        { name: "title", label: "What the job is", required: true, maxLength: 240 },
        { name: "work_order_number", label: "Work order number", maxLength: 120 },
        { name: "customer_id", label: "Customer", type: "reference", from: "customers" },
        { name: "location_id", label: "Where", type: "reference", from: "locations" },
        { name: "booking_id", label: "Booking", type: "reference", from: "bookings" },
        { name: "vehicle_id", label: "Vehicle", type: "reference", from: "vehicles" },
        { name: "route_session_id", label: "Route or site session", type: "reference", from: "routeSessions", hint: "Optional. Link an existing route or job-site tracking session." },
        { name: "priority", label: "Priority", type: "select", options: ["low", "normal", "high", "urgent"] },
        { name: "scheduled_start_at", label: "Scheduled start", type: "datetime-local" },
        { name: "scheduled_end_at", label: "Scheduled end", type: "datetime-local" },
        { name: "agreed_amount_cents", label: "Agreed amount, in pence or cents", type: "number", hint: "1250 means £12.50 or $12.50." },
        { name: "labor_cost_cents", label: "Labour cost recorded, in pence or cents", type: "number" },
        { name: "travel_cost_cents", label: "Travel cost recorded, in pence or cents", type: "number" },
        { name: "other_cost_cents", label: "Other direct cost, in pence or cents", type: "number" },
        { name: "description", label: "Job notes", type: "textarea", maxLength: 2000 }
      ]
    },
    lines: [
      {
        table: "business_work_order_assignments",
        parentColumn: "work_order_id",
        api: "/api/business/work-order-assignments",
        ownedReferences: { employee_id: "business_employee_profiles" },
        title: "Assigned crew",
        empty: "Nobody is assigned to this job yet.",
        columns: [
          { label: "Role", value: (row) => text(row.role_label, "Crew member") },
          { label: "Status", value: (row) => words(row.assignment_status) },
          { label: "Assigned", value: (row) => when(row.assigned_at) }
        ],
        form: {
          legend: "Assign somebody",
          fields: [
            { name: "employee_id", label: "Who", required: true, type: "reference", from: "staff" },
            { name: "role_label", label: "Job role", maxLength: 120 },
            { name: "assignment_status", label: "Assignment", type: "select", options: ["assigned", "accepted", "declined", "completed", "removed"] }
          ]
        }
      },
      {
        table: "business_work_order_materials",
        parentColumn: "work_order_id",
        api: "/api/business/work-order-materials",
        ownedReferences: { inventory_item_id: "inventory_items" },
        requireEither: { reference: "inventory_item_id", fields: ["description"] },
        title: "Materials",
        empty: "No materials are linked to this job yet.",
        columns: [
          { label: "Item", value: (row) => text(row.description || (row.inventory_item_id ? "Inventory item" : ""), "Not described") },
          { label: "Planned", value: (row) => quantity(row.quantity_planned) },
          { label: "Used", value: (row) => quantity(row.quantity_used) },
          { label: "Cost each", value: (row) => money(row.unit_cost_cents) },
          { label: "Status", value: (row) => words(row.material_status) }
        ],
        form: {
          legend: "Add material",
          fields: [
            { name: "inventory_item_id", label: "From inventory", type: "reference", from: "inventory" },
            { name: "description", label: "Description", maxLength: 240 },
            { name: "quantity_planned", label: "Planned quantity", type: "number", step: "0.01" },
            ...
            { name: "material_status", label: "Status", type: "select", options: ["planned", "reserved", "used", "returned", "cancelled"] }
          ]
        }
      },
      {
        table: "business_work_order_evidence",
        parentColumn: "work_order_id",
        api: "/api/business/work-order-evidence",
        person: "recorded_by",
        title: "Completion evidence",
        empty: "No completion evidence has been recorded yet.",
        columns: [
          { label: "Kind", value: (row) => words(row.evidence_type) },
          { label: "Recorded", value: (row) => when(row.captured_at) },
          { label: "Note", value: (row) => text(row.note, "None") }
        ],
        form: {
          legend: "Record evidence",
          fields: [
            { name: "evidence_type", label: "Kind", type: "select", options: ["note", "measurement", "checklist"] },
            { name: "note", label: "What proves the work", type: "textarea", required: true, maxLength: 2000 },
            { name: "captured_at", label: "When", type: "datetime-local" }
          ]
        }
      }
    ],
    derivedReads: async (parent, scopedList) => {
      const time = await scopedList("employee_time_entries", `&work_order_id=eq.${encodeURIComponent(parent.id)}&order=clock_in_at.asc`);
      const invoices = await scopedList("customer_invoices", `&work_order_id=eq.${encodeURIComponent(parent.id)}&order=created_at.desc`);
      const events = await scopedList("business_work_order_events", `&work_order_id=eq.${encodeURIComponent(parent.id)}&order=created_at.desc`);
      let payments = { ok: true, rows: [] };
      const invoice = invoices.ok ? invoices.rows[0] : null;
      if (invoice?.id) payments = await scopedList("customer_invoice_payments", `&invoice_id=eq.${encodeURIComponent(invoice.id)}&order=received_on.desc`);
      return { time, invoices, invoice, payments, events };
    },
    derivedCard: (parent, childRows, ui, extra = {}) => {
      const materials = childRows[1]?.ok ? childRows[1].rows : [];
      const profit = workOrderLifecycle.profitability(parent, materials);
      const timeRows = extra.time?.ok ? extra.time.rows : [];
      const paymentRows = extra.payments?.ok ? extra.payments.rows : [];
      const paid = paymentRows.reduce((sum, row) => {
        const amount = finiteNumber(row.amount_cents);
        return amount === null ? sum : sum + amount;
      }, 0);
      const invoice = extra.invoice;
      const lastEvent = extra.events?.ok ? extra.events.rows[0] : null;
      const profitText = profit.profitCents === null
        ? "Not complete yet. Record agreed revenue and all direct costs before treating profit as known."
        : money(profit.profitCents, parent.currency);
      return `<article class="card"><h2>Job economics and links</h2>
        <p>Agreed revenue: ${ui.escape(money(profit.revenueCents, parent.currency))}. Materials used/planned cost: ${ui.escape(money(profit.materialCostCents, parent.currency))}. Recorded labour: ${ui.escape(money(profit.laborCostCents, parent.currency))}. Travel: ${ui.escape(money(profit.travelCostCents, parent.currency))}. Other direct cost: ${ui.escape(money(profit.otherCostCents, parent.currency))}.</p>
        <p>Direct profit: ${ui.escape(profitText)}</p>
        <p>Time entries linked to this job: ${ui.escape(String(timeRows.length))}. Invoice: ${ui.escape(invoice ? words(invoice.status) : "Not raised")}. Payments recorded against that invoice: ${ui.escape(money(paid, invoice?.currency || parent.currency))}.</p>
        <p>Latest job event: ${ui.escape(lastEvent ? `${words(lastEvent.event_type)} — ${when(lastEvent.created_at)}` : "None recorded yet.")}</p>
      </article>`;
    }
  },
  {
  ...
  },
  {
    ...
    form: {
      ...
      fields: [
        ...
        { name: "notes", label: "Notes", type: "textarea", maxLength: 2000 },
        // Kept as the fulfillment status declaration used by the status
        // control, but omitted from create/edit forms. New orders always start
        // as drafts and the server gates sent/received on owner approval.
        { name: "status", label: "Status", type: "select", create: false, options: ["draft", "sent", "partially_received", "received", "cancelled", "archived"] }
      ]
    }
  },
  {
  ...
  },
  {
    ...
    additionalRowActions: [{
      api: "/api/business/quotes/:id/work-order",
      label: "Start the job",
