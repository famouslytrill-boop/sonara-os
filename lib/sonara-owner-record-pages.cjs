  ...
  },
  {
    ...
    also: [
      ...
      select: "accessibility_notes,event_name,id,name,status",
        ...
        // "You have not defined any vibration patterns yet" told a customer they
        // simply had not got round to it. They cannot: this is an `also` block,
        // no `also` block in this file carries a create form, and the page's one
        // form makes sound cues. The only way to make a vibration pattern is a
        // direct POST to /api/sensory/haptic-patterns.
        //
        // A page inviting an action it does not offer is the same defect as a
        // page claiming a capability it does not have. Saying so also states the
        // AGENTS.md position plainly: haptics stay off until something exists.
        empty: "No vibration patterns are set up, and there is no way to add one from this page. Vibration stays off until one exists.",
        columns: [
