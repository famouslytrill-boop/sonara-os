...
const OWNER_RECORD_PAGES = Object.freeze([
    ...
    lines: [{
      ...
      form: {
        ...
        fields: [
          ...
          // Stored as a percentage, so 5 means 5%. Nothing read this column
          // before, so the convention is set here rather than inherited, and it
          // is written on the field because a customer typing 5 and getting
          // 500% waste is the failure that convention causes.
      ...
      derive: (submitted) => {
        ...
        return { calculated_cost_cents: Math.round(quantityValue * unitCost * wasteMultiplier) };
      }
    }],
  ...
  },
  {
    ...
    body: "A day at a time: what you took, what it cost you and what was left. Working these out needs your daily sales and your labour hours; recording sales is under Daily sales, and the calculation is not built yet.",
    empty: "Nothing here yet, and nothing will appear until the daily figures are calculated -- that work is not built. Recording your sales under Daily sales is the half of it that is ready.",
    columns: [
      { label: "Day", value: (row) => day(row.business_date) },
