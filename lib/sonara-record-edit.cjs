...
function editableFields(page) {
  ...
  // Status has its own validated, audited endpoint. Letting the generic edit
  // form write it as well bypasses transition-specific controls such as
  // purchase-order approval.
  return fields.filter((field) => field && typeof field.name === "string" && field.name && field.name !== "status");
}

function namedFields(page) {
  const fields = page?.form?.fields;
  return Array.isArray(fields)
    ? fields.filter((field) => field && typeof field.name === "string" && field.name)
    : [];
}
...
function changesFrom(page, body, row) {
  const fields = namedFields(page);
  const editable = editableFields(page);
  if (!editable.length) return { ok: false, code: "not_editable", detail: "This kind of record cannot be corrected here." };
  ...
  for (const field of fields) {
    ...
    // Status remains validated for stale or hand-built callers, but never
    // enters this patch. Its dedicated endpoint owns that transition.
