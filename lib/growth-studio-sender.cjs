...
// `quote()` rounds both up to a whole minor unit -- so at 5 emails the charge is
// 1 and the cost is also 1. Seven is where they separate; ten is the documented
// minimum because it is a round number safely past it.
//
...
function consentState(recipient, now, { channel = "email" } = {}) {
  ...
  const consent = recipient?.consent || null;
...
function authoriseCampaign({ approval = null, recipients = [], history = null, allowanceMinor = 0, now = Date.now(), channel = "email" } = {}) {
  ...
  const billableEmails = Math.max(split.eligible.length, MINIMUM_BILLABLE_EMAILS);
