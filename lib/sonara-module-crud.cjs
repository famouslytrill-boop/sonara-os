...
// sharePath and shareUrl rather than a second copy of `/shared/${token}`.
// renderShareControl built that string by hand, which meant the share address
// had two definitions and only one of them checked the token: sharePath()
// returns null for anything that is not a share token, and the hand-built
// version rendered a link for whatever it was given.
const { sharePath, shareUrl } = require("./sonara-shared-results.cjs");
...
function renderShareControl({ record, shared, backHref, origin = "" }) {
  ...
  const path = live?.token ? sharePath(live.token) : null;
  if (path) {
    // What the customer reads is what they can send.
    //
    // This printed the path -- "/shared/abc123" -- which is the one thing a
    // share link must not be. The whole point of the feature is producing
    // something you can paste into a message, and a path pasted anywhere else
    // goes nowhere. `shareUrl` existed for this and was called by nothing in
    // the repository; the page had its own copy of the path instead.
    //
    // The href stays relative either way: the browser resolves it, and it keeps
    // working behind a proxy that rewrites the host. Only the text a person
    // copies needs the origin, and when there is no origin the path is shown
    // rather than a guessed one -- an address invented from a default host is
    // worse than a short one, because it looks sendable and is not.
    const full = origin ? shareUrl(origin, live.token) : null;
    const label = full || path;
    return `<p class="fine">Anyone with this link can read this result: <a href="${escapeHtml(path)}">${escapeHtml(label)}</a></p>
...
// Where a saved result's own tool lives, so it can be reopened with the numbers
// that produced it.
//
// Built from `app.locals.sonaraFreeTools`, which is the same table that
// registered the routes -- not guessed from the module_key, which does not
// contain its own URL and would produce a link that 404s the day a tool moves.
// A tool missing either half is left out, so a half-registered tool loses its
// link rather than gaining a broken one.
function toolPathsFrom(freeTools) {
  return new Map((Array.isArray(freeTools) ? freeTools : [])
    .filter((tool) => tool && tool.module && tool.path)
    .map((tool) => [tool.module, tool.path]));
}

function renderSavedOutputCards({ records, shared, productLabel, backHref, freeTools = null, origin = "" }) {
  const toolPaths = toolPathsFrom(freeTools);
  if (!records.length) {
  ...
  const cards = records.map((record) => {
    ...
    return `<article class="card">
      ...
      ${toolPaths.get(record.module_key)
        ? `<p><a class="action" href="${escapeHtml(toolPaths.get(record.module_key))}?reuse=${escapeHtml(String(record.id))}">Use these numbers again</a></p>`
        : ""}
      ${renderShareControl({ record, shared, backHref, origin })}
    </article>`;
  }).join("");
...
module.exports = { createModuleCrud, RESOURCES, REQUIRED, resourceForForm, renderRecordCards, renderSavedOutputCards, renderShareControl, renderRecordsUnavailable, toolPathsFrom };
