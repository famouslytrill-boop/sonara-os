...
// but the words.
function renderShareControl({ record, backHref }) {
  const id = escapeHtml(String(record.id || ""));
  const back = `<input type="hidden" name="back" value="${escapeHtml(backHref || "/dashboard")}">`;
  const token = String(record.share_token || "");
  if (token) {
    const href = `/shared/${encodeURIComponent(token)}`;
    return `<p class="fine">Anyone with this link can read this result: <a href="${escapeHtml(href)}">${escapeHtml(href)}</a></p>
      <form method="post" action="${escapeHtml(`/api/shared-results/${id}/revoke`)}">${back}<button type="submit">Stop sharing this</button></form>`;
  }
  // shared_at outlives the token on purpose, so a customer who unshared
  // something is told that rather than shown a button that looks untouched.
  const wasShared = record.shared_at
    ? `<p class="fine">This was shared before and is private again.</p>`
    : "";
  return `${wasShared}<form method="post" action="${escapeHtml(`/api/shared-results/${id}/share`)}">${back}<button type="submit">Share this result</button></form>`;
}

function renderSavedOutputCards({ records, productLabel, backHref }) {
  if (!records.length) {
  ...
  const cards = records.map((record) => {
    ...
    return `<article class="card">
      ...
      ${renderShareControl({ record, backHref })}
    </article>`;
  }).join("");
