...
// A boolean field, and three states rather than two.
//
// `reciprocalLicense` is a required field, so a record missing it is a record
// this reader could not understand -- not a record whose answer is false.
// Returning false for an absent field is how an unreadable AGPL entry would
// come to read as permissive, which is the one mistake this field exists to
// stop. `null` means "not recorded" and the gate that reads it treats that as a
// failure.
function booleanField(block, name) {
  const match = block.match(new RegExp(`(?:^|[^\\w-])${KEY(name)}:\\s*(true|false)\\s*,`));
...
function readOpenSourceTools() {
  ...
  return [...source.matchAll(BLOCK)].map((match) => {
    ...
    return {
      ...
      commercialUseStatus: normalizeCommercialUseStatus(field(block, "commercialUseStatus")) || field(block, "commercialUseStatus"),
      integrationStatus: field(block, "integrationStatus"),
      reciprocalLicense: booleanField(block, "reciprocalLicense"),
...
//
// **This is the second copy of this map, and for a week it was the unchecked
// one.** data/open-source-tools.ts holds `openSourceToolStatuses`, which
// scripts/verify-open-source-registry.mjs checks against the
// OpenSourceIntegrationStatus union in both directions. This map is what
// /research-lab/open-source actually renders, through integrationLabel below,
// and nothing compared it to anything.
//
// So when `adapter_built` was added to the union it was added to the checked
// map and not to this one, and `integrationLabel`'s `|| value` fallback made
// the gap silent: **nine records rendered the raw string "adapter_built" on a
// public page.** The verifier's own comment reasoned that the symptom would be
// "a status rendering as undefined wherever the map is read" -- true of the map
// it was looking at, and there were two.
//
// The two maps are allowed to word a status differently, because this one is
// customer-facing prose and that one is not ("Needs licence review" here,
// "Needs license review" there). Neither may be missing a status, and the
// verifier now checks both.
...
// The TypeScript register is parsed as text at runtime, so this small runtime
// contract is intentionally explicit. It keeps customer-facing consumers and
// release validators on the same five canonical values without treating a
// historical spelling as a new policy decision.
const COMMERCIAL_USE_STATUS_VALUES = Object.freeze([
  "allowed",
...
const COMMERCIAL_USE_STATUS_ALIASES = Object.freeze({
  blocked_pending_review: "blocked_until_review",
  not_allowed: "blocked"
});

function normalizeCommercialUseStatus(value) {
  const status = String(value || "").trim();
  if (COMMERCIAL_USE_STATUS_VALUES.includes(status)) return status;
  return COMMERCIAL_USE_STATUS_ALIASES[status] || null;
}
