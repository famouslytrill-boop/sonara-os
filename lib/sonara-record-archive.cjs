...
function canArchive(page) {
  ...
  if (page.lifecycleManaged === true) return false;
...
function reasonWithoutArchive(page) {
  ...
  if (page?.lifecycleManaged === true) {
    return "This record has its own job stages, including cancelled and closed, so it is retired through that lifecycle rather than hidden with a second archive control.";
  }
