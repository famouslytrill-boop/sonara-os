...
// environment-controlled base URL in this module, because redirecting a
// high-authority credential to an arbitrary host is not a feature SONARA needs.
// OpenAI-compatible or local gateways remain separate, governed adapters.
  ...
  try {
    ...
    return {
      ...
      detail: error?.name === "AbortError"
        ? `Anthropic did not answer within ${timeoutMs}ms.`
