...
const CAPABILITIES = Object.freeze({
  ...
  telephony: Object.freeze({
    ...
    //
    // **This floor covers carrying the call. It does not cover ANSWERING it.**
    //
    // An AI receptionist is a second per-minute bill on top of this one, and it
    // dwarfs it: ElevenLabs Agents is $0.08 per agent minute at base rate and
    // $0.160 at burst (elevenlabs.io/pricing/api, read 11 September 2026), so an
    // AI-answered minute costs 8.32 against Telnyx or 8.85 against Twilio --
    // roughly three times this whole PRICE, and six at burst.
    //
    // Billing that through `telephony` would sell it at about a third of cost,
    // and `verifyMargins()` would stay green, because the floor is right for
    // what it was measured against. That is the failure to avoid: not a wrong
    // floor, but a new thing billed through an old capability.
    //
    // So an AI receptionist needs its own capability with its own sourced floor.
    // Nothing offers one today, so nothing is mispriced today.
    // docs/architecture/2026-09-11-WHERE-A-CALL-IS-ANSWERED.md has the options,
    // and forwarding a call to the owner's own mobile -- one inbound plus one
    // outbound minute, 0.82 on Telnyx -- fits inside this price without an agent
    // at all.
    floorMinor: 1.4,
    priceMinor: 3.0,
