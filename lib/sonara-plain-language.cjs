...
// ---------------------------------------------------------------------------
// Whether a generation service can be used at all -- a different question from
// how one job is going, and the one the pages were answering badly.
//
// getProviderReadiness in lib/creator-generation-provider-registry.cjs returns
// an engineering status. Both places that showed it to a creator ran it through
// `display()`, which replaces underscores with spaces and nothing else, so the
// provider list and the provider dropdown read "setup required", "reference
// only" and "external mcp required".
//
// Two things wrong with that. The words are internal, and -- worse -- three of
// them imply the creator should go and do something. Not one of these states is
// theirs to fix: the account owner connects a service, and the rest are
// decisions this product has already made. A label that sends somebody looking
// for a setting that is not on their screen is a worse failure than an ugly
// one, so each detail below says plainly whose job it is, or that it is nobody's.
//
// The plain-language crawl in tests/plain-language.test.js reads this page --
// signed in, it renders 200 and the words are in the text. It said nothing
// because "setup required" and "reference only" are not in
// BANNED_ON_CUSTOMER_PAGES, and they are not there because "setup required"
// appears on twenty-two other pages including the home page, /about and
// /dashboard. Banning it is a copy project across the product rather than a
// list entry, and shipping a rule that fails twenty-two pages is how a rule
// gets switched off instead of followed.
//
// So this fixes the two surfaces where the words were an engineering status
// shown raw, and leaves the vocabulary question recorded rather than pretended
// to be solved.
// ---------------------------------------------------------------------------

const GENERATION_AVAILABILITY = Object.freeze({
  configured: { label: "Ready to use", detail: "You can send work to this now." },
  disabled: { label: "Switched off", detail: "This one is not turned on for your workspace. The account owner can turn it on." },
  setup_required: { label: "Waiting on the account owner", detail: "The account owner has started connecting this and has not finished. Nothing to do from here." },
  external_mcp_required: { label: "Not offered here", detail: "This one is reached through a separate connection this product does not make." },
  research_only: { label: "Not offered here", detail: "Read and understood, and deliberately not connected." },
  worker_candidate: { label: "Not offered here", detail: "This one would run on hardware nobody has set up." },
  reference_only: { label: "Not offered here", detail: "Recorded for reference, and not connected." },
  provider_not_found: { label: "Not available", detail: "We do not recognise that service." }
});

// Two of the eight cannot occur with the registry as it stands, and are kept
// because getProviderReadiness can still return them: `provider_not_found` when
// it is handed nothing, and `reference_only` when a reference-only provider has
// no maturity recorded -- every one of them has, today.
//
// Named rather than quietly carried. The test asserts these two are genuinely
// unreachable as well as asserting the other six are reachable, so an entry
// whose reason has expired fails in the same way a missing label does.
const GENERATION_AVAILABILITY_UNREACHABLE_TODAY = Object.freeze({
  provider_not_found: "getProviderReadiness returns it only when called with no provider, which no caller does.",
  reference_only: "The fallback when a reference-only provider records no maturity. All of them record one."
});

function generationAvailability(status) {
  return GENERATION_AVAILABILITY[String(status || "").trim()]
    || { label: "Not available", detail: "We could not tell whether this one can be used." };
}

function generationAvailabilityLabel(status) {
  return generationAvailability(status).label;
}
...
const GENERATION_FAILURE = Object.freeze({
  ...
  // Not "setup required". A reference-only provider is a repository this
  // project reviewed and deliberately did not connect, so there is no setup to
  // finish -- saying otherwise sends somebody looking for a switch that was
  // never built.
  provider_not_connected: "That tool is not connected here, so it cannot run this. Pick another, or ask us about it.",
  capability_not_supported: "Nothing connected here can do that yet. Nothing was started or charged.",
...
} = require("./sonara-customer-access-model.cjs");

const PLAN_LABELS = Object.freeze(Object.fromEntries(
  Object.entries(CUSTOMER_ACCESS_BY_PLAN_FLOOR).map(([floor, access]) => [floor, access.label])
));

function planLabel(planFloor) {
  return customerPlanLabel(planFloor);
}

function includedFrom(planFloor) {
  return customerInclusionText(planFloor);
}
