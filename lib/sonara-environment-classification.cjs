...
const OPTIONAL_CAPABILITY = new Set([
  ...
  // Which site scripts/report-deployed-commit.mjs asks which build it is
  // serving. Optional because the script defaults to the production address; it
  // exists so the report can be pointed at a stub, which is how its failure
  // case -- an /api/health that has stopped carrying the deployment block -- is
  // exercised without waiting for production to break.
  "SONARA_PRODUCTION_URL",
  "STRIPE_SUCCESS_URL", "STRIPE_CANCEL_URL",
  "SONARA_CRON_SECRET",
  // Signs the per-recipient unsubscribe token in a Growth Studio campaign.
  //
  // Optional rather than required, and the reason matters more than the
  // classification: lib/growth-studio-unsubscribe.cjs derives a signing key
  // from SUPABASE_SERVICE_ROLE_KEY when this is unset, so an unsubscribe link
  // works on deploy with no owner step -- which it has to, because
  // dispatchCampaign refuses to send a campaign it cannot supply a way out of.
  //
  // What setting it BUYS is stability across a rotation. Unsubscribe links sit
  // in inboxes for months, and rotating the service role key invalidates every
  // one signed from it. An owner who rotates that key should set this first.
  ...
  // The annual forms of the three above, added 5 September 2026. Optional for
  // the same reason and one more: an annual plan stays off the pricing page
  // entirely until its price exists, because the monthly twin already sells the
  // same product. Nobody is shown a card they cannot buy.
  ...
  // their own dashboard.
  // The ten external service adapters, added to this list on 8 September 2026.
  //
  // Not one of these 44 names was classified before that date, and
  // scripts/verify-env.mjs reported "all classified" throughout. They were
  // invisible to it: every adapter names its variables through
  // `base.envKeysFor(PREFIX, [...])`, which builds them by concatenation, so no
  // pass that matches string literals could ever see one. The scanner now has a
  // pass for that shape, and it surfaced all 44 at once.
  //
  // Optional is the right classification and it is enforced rather than
  // asserted. docs/architecture/EXTERNAL-SERVICES.md states the two rules that
  // make it true -- off by default, and never a dependency -- and
  // tests/service-adapters.test.js runs both against every adapter in lib/. A
  // customer is served identically whether all ten are configured or none is.
  //
  // Cloudflare's two are optional for one further reason worth naming: they are
  // a price, not a licence. CLAUDE.md's rule is that a hosted service with a
  // free tier is a price, and a shipped feature resting on one stops working
  // when the tier changes -- which is Cloudflare's decision, not this project's.
  ...
  // R2 carries a key *pair* rather than a bearer token, which is why there are
  // two credential variables here and one everywhere above. Both are secrets:
  // an access key id is not a password but it is half of one, and no page in
  // this product needs to show it.
