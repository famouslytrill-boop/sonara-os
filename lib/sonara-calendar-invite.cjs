...
// for.
//
// ## Three of the fields read below are not columns of that table
//
// `service_name`, `location_name` and `calendar_sequence` are supplied by
// `lib/sonara-booking-calendar-fields.cjs`, which resolves the first two from
// the booking's `service_id` and `location_id` and derives the third from its
// timestamps. **A caller that passes an undecorated row gets a calendar entry
// titled "Booking", with no location and SEQUENCE 0**, which is what every
// download did until 15 September 2026 -- the reads returned `undefined` and
// each fallback was silent. That module's header has the whole account; the
// reason it went unseen for a week is that the query was `select=*`, so nothing
// compared the field names read against the column names asked for.
//
...
// The VEVENT block on its own, so one booking and a whole diary are built by
// the same code. They were not, at first -- the feed was going to assemble its
// own lines -- and two builders of one format drift until a client accepts the
// download and rejects the feed, or the other way round.
function eventLines(booking, verdict, settings, now) {
...
// The wrapper every calendar file needs, whatever is inside it.
function calendarLines(events) {
  return [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
...
function buildCalendarInvite(booking, options) {
  ...
  return {
    ...
    body: render(calendarLines(eventLines(booking, verdict, settings, now)))
  };
}

// A diary rather than one appointment.
//
// The per-booking download shipped without this and was half a feature: a
// business wants their week in their calendar, not to click twenty times.
//
// What it does with a booking a calendar cannot show is the part worth stating.
// It skips it and *counts* it, and the count is returned rather than dropped,
// because a feed that quietly omits three appointments is a diary that lies by
// being incomplete -- and the business would have no way to know. The caller is
// expected to say so.
function buildCalendarFeed(bookings, options) {
  const settings = options && typeof options === "object" ? options : {};
  const rows = Array.isArray(bookings) ? bookings : null;
  // null and [] are different answers. A read that failed must not render as a
  // business with no bookings.
  if (!rows) {
    return { ok: false, code: "not_a_list", message: "We could not read your bookings just now, so this calendar was not built." };
  }
  ...
  for (const booking of rows) {
    const verdict = invitability(booking);
    if (!verdict.ok) {
      skipped.push({ id: booking && booking.id ? String(booking.id) : null, code: verdict.code, message: verdict.message });
      continue;
    }
    events.push(...eventLines(booking, verdict, settings, now));
  }

  return {
    ok: true,
    // An empty diary is a real answer and still a valid calendar file: a
    // VCALENDAR with no VEVENT parses, and imports nothing, which is correct.
    included: events.length ? rows.length - skipped.length : 0,
    skipped,
    filename: "bookings.ics",
    contentType: "text/calendar; charset=utf-8",
    body: render(calendarLines(events))
  };
}

module.exports = { buildCalendarInvite, buildCalendarFeed, invitability, escapeText, foldLine, toUtcStamp, calendarStatus, uidFor };
