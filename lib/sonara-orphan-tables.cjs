...
const ORPHAN_DISPOSITIONS = Object.freeze({
  ...
  entities: Object.freeze({
    ...
    decision: "keep",
    note:
      "slug, name, entity_type, is_public -- and no organization_id. The root of a second tenancy model: ownership is entity_memberships keyed on entity_id, while everything shipped resolves through organizations. Read it before proposing any 'workspace' rework; do not query it alongside organization-scoped tables."
  }),
  ...
  entity_agents: Object.freeze({
    group: "entity-system",
    decision: "keep",
    note:
      "name, agent_type, permissions_json, config_json. Per-entity agents with permissions stored as JSON. lib/sonara-agent-authority.cjs is the shipped answer and is deliberately code rather than data -- a permission set a row can widen is a permission set that can be widened without review. Do not replace the authority module with this."
  }),
  entity_automations: Object.freeze({
    group: "entity-system",
    decision: "keep",
    note:
      "schedule_cron, trigger_type, action_template_json, requires_approval. Scheduled unattended work. Nothing in this product runs on a schedule and nothing consumes an approval -- AGENTS.md and the runner both stop at recording a pending action -- so this table describes a runtime that does not exist."
  }),
  entity_connectors: Object.freeze({
    group: "entity-system",
    decision: "keep",
    note:
      "connector_type, display_name, config_json. business_integration_connections is the live equivalent and is organization-scoped. Two tables for one idea; the live one already works."
  }),
