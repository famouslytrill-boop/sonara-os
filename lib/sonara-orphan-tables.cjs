...
const ORPHAN_DISPOSITIONS = Object.freeze({
  ...
  // The four _lines tables that were here are queried now. Each is reachable
  // through the record it belongs to -- /business-builder/owner/invoices/:id,
  // /purchase-orders/:id, /stock-counts/:id, /transfers/:id -- which is what
  // "build-with-parent" meant. A standalone page for them was never the plan: a
  // line detached from its order or count is an orphaned row.
