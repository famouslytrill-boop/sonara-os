...
// The classified set is not empty: platform_job_events is server-only
// infrastructure, while translation_records and translation_glossary_terms
// are a tenant-scoped foundation whose application reader/editor is still
// pending. Those decisions are explicit rather than being mistaken for shipped
// customer workflows. The gate computes the set from migrations and runtime
// source on every run; any new unclassified table fails `--check`.
//
// Historical count: it held 206 (claimed), then 90 (measured), then 87, 36, 32,
// 19 and none before these three newer schema additions. Thirteen superseded
// tables were dropped. The earlier feature tables were built: three became
...
const ORPHAN_DISPOSITIONS = Object.freeze({
  ...
  // build.
  //
  // **The owner settled it on 19 August 2026: delete.** The app/ directory, its
  // components, and every .ts module only it imported are gone. The tables
  // stayed, because a table is not a page -- dropping data on the strength of a
  // deleted reader is the destructive change AGENTS.md puts behind owner
  // approval, and these hold a design worth reading before anything similar is
  // built again.
  //
  ...
  // never queried by anything that ran either; they were queried by
  // lib/entities/*.ts and app/dashboard/entities/**, so this check counted them
  // as used right up until those files went. Same tables, same silence, one
  // fewer place to hide.
  // ---------------------------------------------------------------------------
  // ---------------------------------------------------------------------------
  // The remaining entity system orphans: six of the nineteen entity_* tables from migration 008.
  //
  // A parallel model of the whole product. `entities` has slug, name,
  // entity_type and is_public but **no organization_id** -- membership is
  // entity_memberships keyed on entity_id -- so it is a second tenancy scheme
  // beside organizations, not a feature inside it. That is why none of these can
  // be "just wired up": doing so would give this product two answers to the
  // question of who owns a row, which is the question every other check here
  // exists to keep one answer to.
  //
  ...
  // ---------------------------------------------------------------------------
  entity_memberships: Object.freeze({
    group: "entity-system",
    decision: "keep",
    note:
      "entity_id, user_id, role, unique on the pair. The membership table for the model above. organization_memberships is the live one and the only one any read path uses."
  }),
  entity_browser_sessions: Object.freeze({
    group: "entity-system",
    decision: "keep",
    note:
      "start_url, current_url, status -- a record of an agent browsing on somebody's behalf. Nothing here browses: the one fetch path is /api/market-intelligence/fetch-source, which is per-request, gated on an approved research_sources row, and stores nothing."
  }),
  entity_heartbeats: Object.freeze({
    group: "entity-system",
    decision: "keep",
    note:
      "heartbeat_type, health_score, checked_at. Stored health readings. /readiness and scripts/verify-production-supabase.mjs answer the same question live, which is the better shape -- a stored health score is stale the moment it is written."
  }),
  entity_health_snapshots: Object.freeze({
    group: "entity-system",
    decision: "keep",
    note:
      "total_score, status, summary. The rolled-up version of the heartbeats above, and stale for the same reason."
  }),
  entity_incidents: Object.freeze({
    group: "entity-system",
    decision: "keep",
    note:
      "severity, opened_at, resolved_at. An incident log for the entity model. Nothing opens or closes one, and an incident record nobody writes reads as 'no incidents' rather than as 'not tracked'."
  }),
  entity_proactive_actions: Object.freeze({
    group: "entity-system",
    decision: "keep",
    note:
      "action_type, priority, proposed_by, assigned_to, due_at -- an agent proposing work to a person. agent_pending_actions is the shipped approval record and is organization-scoped. Nothing consumes an approval in either table, which is stated in CLAUDE.md and remains true."
  }),
  ...
  platform_job_events: Object.freeze({
    group: "server-infrastructure",
    decision: "keep",
    note:
      "Service-role-only lifecycle records for durable platform jobs. This is infrastructure evidence, not a customer workspace; retain it for worker rollout and audit until the current outbox/PGMQ consumer either adopts it or supersedes it."
  }),
  translation_records: Object.freeze({
    group: "translation-foundation",
    decision: "build",
    note:
      "Tenant-scoped, human-editable source and target messages. The schema does not claim machine translation; the application reader/editor remains to be connected to the deterministic catalog workflow."
  }),
  translation_glossary_terms: Object.freeze({
    group: "translation-foundation",
    decision: "build",
    note:
      "Tenant-scoped approved terminology for deterministic substitutions. The data contract is in place; glossary management and runtime use remain to be connected before this becomes a customer-facing feature."
  }),
});
