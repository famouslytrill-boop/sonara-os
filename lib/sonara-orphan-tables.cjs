...
// Five tables re-orphaned by deleting routes/creator-artist-system-routes.cjs.
//
// That module was the only code that read them, and it was required by nothing
// -- so its routes 404ed and these tables were never actually reached in
// production either. They read as used only because a file existed that would
// have used it, had anything loaded the file.
//
// Not retired, because retiring is a schema change and this is a code deletion.
// Recorded so the next person sees a decision rather than a gap, and so the
// choice -- build the artist workspace properly, or drop the tables -- is made
// deliberately rather than inherited.
const ORPHAN_DISPOSITIONS = Object.freeze({
  creator_artist_profiles: Object.freeze({
    group: "buildable",
    decision: "keep",
    note:
      "Read only by routes/creator-artist-system-routes.cjs, which server.js never registered -- so the pages 404ed and these rows were never written in production. If a creator artist workspace is wanted it should be built like the other record pages, from the real columns, rather than by re-wiring a module whose template hardcoded an invented artist."
  }),
  creator_album_cycles: Object.freeze({
    group: "buildable",
    decision: "keep",
    note: "Same deleted module. An album cycle belongs to an artist profile, so the two are one decision rather than two."
  }),
  creator_prompt_blueprints: Object.freeze({
    group: "buildable",
    decision: "keep",
    note: "Same deleted module. A prompt blueprint hangs off an artist system, and the Prompt Library already stores templates with versions -- so if this is built, the first question is whether it is a second prompt store or a view onto the one that exists."
  }),
  creator_sonic_profiles: Object.freeze({
    group: "buildable",
    decision: "keep",
    note: "Same deleted module. Belongs to an artist profile, so it is part of the same one decision."
  }),
  creator_video_treatments: Object.freeze({
    group: "buildable",
    decision: "keep",
    note: "Same deleted module. Belongs to an artist profile, so it is part of the same one decision."
  })
