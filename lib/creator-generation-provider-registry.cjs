...
const PROVIDERS = Object.freeze([
  ...
  cloud({
    ...
    capabilities: ["text_to_image", "image_to_image", "image_edit", "text_to_video", "image_to_video", "video_to_video", "text_to_music", "text_to_audio", "text_to_speech", "speech_to_speech", "reference_analysis"],
...
// A reference-only provider is the record of a repository somebody reviewed, not
// an adapter. It carries no `enabledEnv`, no `baseUrlEnv` and no `requiredEnv`,
// so `getProviderReadiness` returns `configured: false` for every environment
// there will ever be, and no amount of setup changes that.
//
// Choosing one anyway produced a job with status `setup_required`, which tells a
// customer to finish a setup that cannot be finished. `lib/sonara-plain-language.cjs`
// has rendered `reference_only` as "Not offered here. Recorded for reference,
// and not connected" the whole time, so one registry was giving two answers.
//
// This is the defect the FORM_CAPABILITY_ORDER comment in
// routes/creator-generation-routes.cjs describes, one layer down. There the menu
// offered `voice_clone` and `singing_voice` that no provider declared, and a
// customer recorded a permission naming a real person before finding out it
// could not run. The fix was to filter the menu by what a provider declares --
// but a reference-only provider *does* declare capabilities, and 23 of them
// reach the API that way, `voice_cloning` and `voice_conversion` among them.
//
// growth-studio-provider-registry.cjs already excludes them from its candidates.
// This brings the two registries into agreement rather than adding a new rule.
function isSelectable(provider) {
  return provider.adapterMode !== "reference_only";
}
...
function chooseProvider(capability, requestedKey, env = process.env) {
  if (requestedKey && requestedKey !== "auto") {
    ...
    // Refused rather than accepted-and-parked. Asking for ComfyUI by name is a
    // clear request, and "not connected here" is the true answer to it.
    if (!isSelectable(requested)) return { ok: false, code: "provider_not_connected" };
    return { ok: true, provider: requested, readiness: getProviderReadiness(requested, env) };
  }
  const candidates = PROVIDERS.filter((provider) => provider.capabilities.includes(capability) && isSelectable(provider));
