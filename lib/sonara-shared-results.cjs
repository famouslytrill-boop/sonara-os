...
function invoiceView(row, lines = [], settlement = null) {
  ...
  // The balance, when the caller worked one out. Placed under the total rather
  // than replacing it: a customer checking an invoice against their own records
  // needs the figure they were originally charged as well as the one left.
  //
  // "Still owed" is deliberately absent rather than shown as the total when the
  // payments could not be read. A page that guesses here tells somebody to pay
  // an invoice they have already settled.
  if (settlement && settlement.status !== "unpriced") {
    if (settlement.status === "unknown") {
      detail.push({ label: "Still owed", value: "We could not check what has been paid" });
    } else {
      if (settlement.paidCents > 0) detail.push({ label: "Paid so far", value: money(settlement.paidCents, currency) || "Not set" });
      if (settlement.status === "overpaid") {
        detail.push({ label: "Overpaid by", value: money(settlement.overpaidCents, currency) || "Not set" });
      } else {
        detail.push({
          label: "Still owed",
          value: settlement.outstandingCents === 0
            ? (settlement.certain ? "Nothing -- paid in full" : "Nothing left on our records")
            : (money(settlement.outstandingCents, currency) || "Not set")
        });
      }
      if (settlement.note) detail.push({ label: "Please note", value: settlement.note });
    }
  }
  ...
  return {
    ...
    // Unchanged in substance, and deliberately.
    //
    // A shared invoice now shows a balance, which makes it more useful and not
    // more payable. There is no pay button here and there will not be one until
    // this application can route the money to the business rather than to
    // itself -- see the note at the top of lib/sonara-invoice-settlement.cjs.
    // The advice only protects anybody if it is always true: a product where
    // some invoice links have a pay button has taught its customers that a pay
    // button on an invoice link is normal, which is the shape of the fraud.
 ...
 */
function sharedView({ resourceType, row, lines = [], organizationName = "", settlement = null } = {}) {
  const shareable = shareableFor(resourceType);
  if (!shareable || !row || typeof row !== "object") return null;
  // The third argument is only meaningful for an invoice; every other view
  // ignores it. Passed unconditionally rather than branched on the type, so a
  // future kind that grows a balance does not have to remember to be added here.
  const view = shareable.view(row, lines, settlement);
  return {
    ...view,
    noun: shareable.noun,
