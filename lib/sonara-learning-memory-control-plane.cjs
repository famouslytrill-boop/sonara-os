...
// feature. The repository already contains an older user-scoped vector-memory
// schema and dormant entity-agent memory tables. This module prevents those
// historical artifacts from being described as a live, organization-scoped
// semantic-memory product before a reviewed runtime and migration actually
// exist. It intentionally does not name the legacy table as a runtime field:
// the orphan-table gate treats a runtime string naming a table as evidence that
// code uses the table, and this policy module never queries it.
...
function getLearningMemoryControlPlane(env = process.env) {
  ...
  return {
    ok: true,
    dataSource: "repository_static_policy",
    ...
    currentState: {
      ...
      legacyVectorMemorySchema: {
        ...
        artifact: "legacy user-scoped vector-memory migration",
      ...
      entityAgentMemory: {
        ...
        artifact: "legacy entity-agent memory schema",
