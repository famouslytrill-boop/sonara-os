...
// against a dated and sourced floor. `lib/sonara-usage-meter.cjs`
...
const CAPABILITY_ESTIMATES = Object.freeze({
  // Image jobs have no duration axis. Until the media worker reports measured
  // compute, these are explicit flat authorization estimates rather than fake
  // metering. Production activation still requires the worker/operator cost
  // review documented in MEDIA-WORKER-INSTALL.md.
  text_to_image: Object.freeze({ gpuSecondsPerOutputSecond: 0, defaultOutputSeconds: 0, flatGpuSeconds: 16 }),
  image_to_image: Object.freeze({ gpuSecondsPerOutputSecond: 0, defaultOutputSeconds: 0, flatGpuSeconds: 16 }),
  image_edit: Object.freeze({ gpuSecondsPerOutputSecond: 0, defaultOutputSeconds: 0, flatGpuSeconds: 16 }),
...
//
// The price list works in minor units of a cent, and `quote()` rounds both the
// charge and the cost UP to a whole one. At small quantities that rounds them to
// the same number: the job runs at exactly zero margin, which is worse than the
// rounding it came from. Anything below this minimum is billed as this minimum.
//
// The price and the floor are deliberately not repeated here, and the reason for
// the number is now a check rather than a sentence. This comment used to quote
// both figures AND claim that eight was "the smallest quantity where the two
// separate" -- arithmetic that reads like it was run and was not. The real
// separation point on the current price list is five. Eight stays, because a
// minimum comfortably above the separation point is the safe direction, but
// `tests/a-cost-floor-is-a-figure-somebody-checked.test.js` now derives that
// point from the price list and fails if this drops to or below it.
