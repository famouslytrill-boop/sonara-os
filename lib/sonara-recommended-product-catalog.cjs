...
function expand(productKey, row) {
  ...
  //
  // An intermediate implementation asked only whether the product family
  // enforced some paid entitlement. That was still too broad: a catalogue floor
  // could advertise access that the family's actual billing entitlement would
  // not open, producing a 402 after a customer bought the advertised level.
  //
  ...
  const customerAccess = customerAccessForPlanFloor(planFloor);
  const workflowState = workflowTruth({ lifecycleStatus, executionEnabled });
  return {
    productKey,
    serviceKey,
    name,
    ...
    customerOutcome,
    planFloor,
    // Customer-facing access stays on the current breadth ladder even while
    // planFloor preserves the historical database contract.
    accessClass: customerAccess?.accessClass || "paid",
    workspaceScope: customerAccess?.workspaceScope || "all",
    customerPlan: customerAccess?.label || "Paid",
