...
const PROVIDERS = Object.freeze([
  ...
  planned({
    key: "google_ads_data_manager",
    label: "Google Ads Data Manager",
    officialUrl: "https://developers.google.com/data-manager/api/devguides/events/google-ads/offline",
    capabilities: ["offline_conversion_ingest", "enhanced_conversion_leads"],
    maturity: "research_only",
    notes: [
      "New offline-conversion integrations should use the Data Manager API contract rather than assuming the legacy Google Ads API upload path.",
      "Conversion data must preserve consent, source, event time, deduplication, and upload-result evidence.",
      "No audience or conversion data is transmitted until an approved connector and real-account acceptance test exist."
    ]
  }),
  planned({
    key: "google_search_console",
    label: "Google Search Console",
    officialUrl: "https://developers.google.com/webmaster-tools/v1/api_reference_index",
    capabilities: ["search_performance", "url_inspection", "sitemap_read", "site_read"],
    maturity: "research_only",
    notes: [
      "Use read-first SEO evidence for queries, pages, devices, countries, indexing status, and sitemap health.",
      "Search Analytics can omit rows because of provider limits, so Growth Studio must retain coverage and freshness caveats.",
      "Search performance is evidence, not a guarantee of rankings or causal lift."
    ]
  }),
  planned({
    key: "youtube_data",
    label: "YouTube Data",
    officialUrl: "https://developers.google.com/youtube/v3/guides/uploading_a_video",
    capabilities: ["video_upload", "publish_status", "channel_content_sync"],
    maturity: "research_only",
    notes: [
      "Video upload must use resumable transfer and bounded retry/backoff behavior.",
      "Public publishing remains a human-approved action even after OAuth is configured.",
      "Media creation belongs to Creator Studio; Growth Studio owns campaign context, scheduling, approval, publishing evidence, and measurement."
    ]
  }),
  planned({
    key: "app_store_connect_analytics",
    label: "App Store Connect Analytics",
    officialUrl: "https://developer.apple.com/help/app-store-connect-analytics/overview/analytics-reports-api",
    capabilities: ["app_acquisition", "app_engagement", "iap_analytics", "subscription_analytics", "cohort_analysis"],
    maturity: "research_only",
    notes: [
      "Analytics ingestion should preserve Apple privacy thresholds, availability gaps, report period, and generation time.",
      "Start read-only; app release and listing mutations belong behind a separate approval boundary.",
      "Join app-store acquisition data to SONARA campaign and conversion evidence without claiming deterministic causation."
    ]
  }),
  planned({
    key: "google_play_reporting",
    label: "Google Play Developer Reporting",
    officialUrl: "https://developers.google.com/play/developer/reporting",
    capabilities: ["android_vitals", "crash_rate", "anr_rate", "slow_rendering", "error_reports", "release_quality"],
    maturity: "research_only",
    notes: [
      "Use app-quality evidence to connect acquisition campaigns with actual product reliability.",
      "Reporting is read-only in the first implementation wave; publishing and release-track mutations require separate approval-gated contracts.",
      "Preserve app version, device, country, time range, and pagination metadata with every snapshot."
    ]
  }),
  reference({
...
function planned(record) {
  return Object.freeze({
    runtimeClass: "cloud_provider_reference",
    adapterMode: "reference_only",
    integrationStatus: record.maturity || "research_only",
...
function chooseGrowthProvider(capability, requestedKey, env = process.env) {
  if (requestedKey && requestedKey !== "auto") {
    ...
    if (provider.adapterMode === "reference_only") return { ok: false, code: "provider_not_runnable" };
