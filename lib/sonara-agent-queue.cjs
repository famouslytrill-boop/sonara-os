 ...
 */
async function approveAndRun({ pending, decidedBy, runner, context = {}, organizationId = null }) {
  ...
  // Attribution forwarded explicitly. The runner does not read `context`, so
  // an approved run that arrived here without an organization would emit a
  // rejected-event line rather than a misattributed one.
  const run = await runner.run({ action, approval, context, organizationId: organizationId || context.organizationId || null });
