...
// legalPages reads the legal document set and safeListTable reads the database.
// The card and link
...
const { UI_LOCALES } = require("./sonara-locale-contract.cjs");
const {
  brandCard,
  escapeHtml,
  linkAction
...
// readinessStatusClass was required here until 8 September 2026 and is not any
// more. It was injected for renderInterfaceStatusPanel, which nothing rendered;
// with that gone, this module had a required dependency no line of it used, and
// every caller was still made to supply one.
//
// I first wrote here that the function itself stays in server.js "where the
// readiness screens use it". That was wrong, and lint said so within the
// minute: the panel was its only caller anywhere, so removing the panel left
// the function dead too, and it went with it. A reason that sounds verified and
// is not is the thing CLAUDE.md warns about, and it took thirty seconds to
// produce one.
const REQUIRED = ["legalPages", "safeListTable"];

function createPageFrame(deps = {}) {
  for (const name of REQUIRED) {
    if (typeof deps[name] !== "function") throw new TypeError(`createPageFrame requires ${name}`);
  }
  const { legalPages, safeListTable, getReadiness } = deps;

  function homePreviewStatuses() {
    const fallback = {
      signal: "Setup review",
      statuses: [
        ["Account", "Sign in to begin"],
    ...
    if (typeof getReadiness !== "function") return fallback;
    try {
      const readiness = getReadiness();
      const services = readiness?.services || {};
      const state = (value, readyLabel = "Ready") => value === "configured" || value === "enabled" ? readyLabel : "Setup required";
      const statuses = [
        ["Account", "Sign in to begin"],
        ["Database", state(services.accountDatabase || services.supabase)],
        ["Payments", state(services.checkout, "Checkout ready")],
        ["Support", state(services.emailDelivery, "Support ready")],
        ["Operations", state(services.adminProtection, "Protected")]
      ];
      const needsSetup = statuses.some(([, value]) => value === "Setup required");
      return { signal: needsSetup ? "Setup review" : "Ready to work", statuses };
    } catch {
      return fallback;
    }
  }
  ...
  function renderHead(title, themeColor = "#04050B", body = "", canonical = "") {
  ...
  // renderInterfaceStatusPanel was removed on 8 September 2026: 14 lines of a
  // status panel that nothing rendered. Same shape as renderHomepageContent
  // above it -- returned by createPageFrame, destructured by nobody, called by
  // nothing -- and found by scripts/report-uncalled-factory-functions.mjs,
  // which exists because the first one was found by accident.
  ...
  // `scripts` is a list of same-origin paths a page needs on top of the shell's
  // own. It exists because the Content-Security-Policy here is
  // `script-src 'self'` with no bundler, so a page that needs behaviour needs a
  // file -- and the alternative was every page loading every script in case one
  // of them wanted it.
  function layout({ title, eyebrow, heading, body, sections, actions, authenticated = false, headingI18nKey = "", bodyI18nKey = "", variant = "standard", surface = "work", canonical = "", scripts = [] }) {
    const brandClass = pageBrandClass(title, heading, eyebrow);
    const isMarketing = surface === "marketing";
    const pageActions = Array.isArray(actions) ? actions : [];
    const hasLegacyLogoutAction = pageActions.some((action) => /<form\b[^>]*action="\/logout"/i.test(String(action)));
    const linksToAccount = pageActions.some((action) => /href="\/account(?:\/|\")/i.test(String(action)));
    const isAuthenticated = authenticated || hasLegacyLogoutAction || linksToAccount;
    // Sign-out belongs in the persistent account menu. Keep older route
    // modules working while removing their duplicate in-page sign-out links.
    const renderedActions = pageActions.filter((action) => !/<form\b[^>]*action="\/logout"/i.test(String(action)));
    const mainClass = isMarketing ? "sonara-ds sonara-stage" : "sonara-ds";
    const localeOptions = UI_LOCALES.map(({ code, language }) => `<option value="${escapeHtml(code)}" lang="${escapeHtml(code)}">${escapeHtml(language)}</option>`).join("");
    let renderedSections = sections.join("");
    if (variant === "home") {
      // Keep the public path focused on workspace entry. The legacy request
      // endpoint remains available for integrations, but customers should not
      // be funnelled into a form before they can start working.
      renderedSections = renderedSections
        .replaceAll('href="/business-builder/intake"', 'href="/business-builder/launch-readiness"')
        .replaceAll("Open customer intake", "Open launch checklist")
        .replaceAll('href="/business-builder/dashboard"><strong>Business Builder</strong><span>Start an intake</span>', 'href="/business-builder/dashboard"><strong>Business Builder</strong><span>Open your workspace</span>')
        .replace('<a class="action" href="/signup">Create free account</a><a class="action" href="#companies-heading">Explore the studios</a>', '<a class="action" href="/signup">Create free account</a><a class="action" href="/dashboard">Start working</a><a class="action" href="#companies-heading">Explore the studios</a>');
    }
    const preview = variant === "home" ? homePreviewStatuses() : null;
    return `<!doctype html>
  <html lang="en">
    <head>
      ${renderHead(title, "#04050B", body, canonical)}
      <link rel="preload" href="/fonts/geist-latin.woff2?v=sonara-ui-20260914-v12-palette" as="font" type="font/woff2" crossorigin>
      <link rel="stylesheet" href="/sonara-fonts.css?v=sonara-ui-20260914-v12-palette">
      <script src="/sonara-prepaint.js?v=sonara-ui-20260914-v12-palette"></script>
      <link rel="stylesheet" href="/sonara-design-system.css?v=sonara-ui-20260914-v12-palette">
      <link rel="stylesheet" href="/sonara-application-ui.css?v=sonara-ui-20260914-v12-palette">
      <script defer src="/sonara-one.js?v=sonara-ui-20260914-v12-palette"></script>
      <script defer src="/sonara-experience-controls.js?v=sonara-ui-20260914-v12-palette"></script>
      ${(Array.isArray(scripts) ? scripts : [])
        // Same-origin paths only. These come from a route rather than from a
        // request, but a list that would accept an absolute URL is one somebody
        // later passes a request value into.
        .filter((src) => typeof src === "string" && /^\/[A-Za-z0-9._\-/]+\.js$/.test(src))
        .map((src) => `<script defer src="${src}"></script>`)
        .join("")}
      ${isMarketing ? `<script defer src="/sonara-depth.js?v=sonara-ui-20260914-v12-palette"></script>` : ""}
    </head>
    <body class="${escapeHtml(brandClass)} ${variant === "home" ? "sonara-home-v3" : "sonara-standard-page"}">
      ...
      <header class="sonara-site-header">
        ...
        <nav class="sonara-desktop-nav" aria-label="Primary">
          ${isAuthenticated
            ? `<a href="/dashboard" data-i18n="workspaces">Workspaces</a><a href="/free-tools" data-i18n="tools">Free tools</a><a href="/pricing" data-i18n="pricing">Pricing</a>`
            : `<a href="/start" data-i18n="productsNav">Products</a><a href="/free-tools" data-i18n="tools">Free tools</a><a href="/pricing" data-i18n="pricing">Pricing</a><a href="/login" data-i18n="login">Sign in</a><a class="sonara-nav-primary" href="/signup" data-i18n="start">Start free</a>`}
        </nav>
        <div class="sonara-header-tools">
          ...
          <button type="button" class="sonara-icon-button" data-sonara-settings aria-haspopup="dialog" aria-controls="sonara-settings-dialog" aria-label="Open experience settings"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 8h3.9M12.1 8h7.9M4 16h9.9M18.1 16h1.9" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/><circle cx="10" cy="8" r="2.1" fill="none" stroke="currentColor" stroke-width="1.8"/><circle cx="16" cy="16" r="2.1" fill="none" stroke="currentColor" stroke-width="1.8"/></svg><span class="sonara-tool-label" data-i18n="experience">Experience</span></button>
          ${isAuthenticated ? `<details class="sonara-account-menu">
          <summary aria-label="Open account navigation" data-i18n="account">Account</summary>
          <div class="sonara-account-panel">
            <a href="/account" data-i18n="account">Account</a>
            <a href="/account/preferences" data-i18n="preferences">Preferences</a>
        ...
        <details class="sonara-mobile-menu"><summary aria-label="Open navigation" data-i18n="menu">Menu</summary><nav aria-label="Mobile primary">
          ${isAuthenticated
            ? `<a href="/dashboard" data-i18n="workspaces">Workspaces</a><a href="/free-tools" data-i18n="tools">Free tools</a><a href="/pricing" data-i18n="pricing">Pricing</a><a href="/account" data-i18n="account">Account</a><a href="/account/preferences" data-i18n="preferences">Preferences</a><a href="/support" data-i18n="support">Support</a><form method="post" action="/logout"><button type="submit" data-i18n="logout">Log out</button></form>`
            : `<a href="/start" data-i18n="productsNav">Products</a><a href="/free-tools" data-i18n="tools">Free tools</a><a href="/pricing" data-i18n="pricing">Pricing</a><a href="/login" data-i18n="login">Sign in</a><a class="sonara-nav-primary" href="/signup" data-i18n="start">Start free</a>`}
        </nav></details>
      </header>
      <main id="sonara-main" class="${mainClass}" data-sonara-surface="${isMarketing ? "marketing" : "work"}" data-sonara-interface="live" data-layout-contract="grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); overflow-wrap: anywhere; word-break: break-word">
        <section class="hero sonara-hero-stage">
          <div class="sonara-hero-copy${isMarketing ? " sonara-reveal" : ""}">
            ...
            <h1${variant === "home" ? ' data-i18n="heroHeading"' : headingI18nKey ? ` data-i18n="${escapeHtml(headingI18nKey)}"` : ""}>${escapeHtml(heading)}</h1>
            <p class="lede"${variant === "home" ? ' data-i18n="heroBody"' : bodyI18nKey ? ` data-i18n="${escapeHtml(bodyI18nKey)}"` : ""}>${escapeHtml(body)}</p>
            ${renderedActions.length ? `<div class="actions">${renderedActions.join("")}</div>` : ""}
          </div>
          ${variant === "home" ? `<div class="sonara-hero-scene" aria-label="SONARA One workspace preview">
            <div class="sonara-interface-preview">
              <div class="sonara-interface-preview__topline"><span class="sonara-interface-preview__eyebrow">SONARA ONE / WORKSPACE</span><span class="sonara-interface-preview__signal">${escapeHtml(preview.signal)}</span></div>
              <div class="sonara-interface-preview__heading"><strong>Next useful action</strong><span>Choose a workspace to begin</span></div>
              <div class="sonara-interface-preview__statuses" aria-label="Workspace readiness">${preview.statuses.map(([label, value]) => `<div><span class="sonara-status-dot" aria-hidden="true"></span><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong></div>`).join("")}</div>
              <div class="sonara-interface-preview__footer"><span>Build</span><span>Create</span><span>Grow</span><a href="/readiness">View setup</a></div>
            </div>
          </div>` : ""}
        </section>
        ${variant === "home" ? `<section class="sonara-home-quickstart" aria-label="SONARA One quickstart">
          <div class="sonara-home-quickstart__intro"><span class="sonara-kicker">Choose your next workspace</span><h2>Start with a real workflow.</h2><p>Open the company that fits today’s work, or review the shared setup before you connect payments, records, or providers.</p></div>
          <div class="sonara-home-quickstart__grid">
            <a class="sonara-quickstart-card sonara-quickstart-card--business" href="/business-builder/dashboard"><strong>Business Builder</strong><span>Open your workspace</span><small>Offers, customers, bookings, and payments.</small></a>
            <a class="sonara-quickstart-card sonara-quickstart-card--creator" href="/creator-studio/assets"><strong>Creator Studio</strong><span>Open assets</span><small>Assets, rights notes, releases, and creator work.</small></a>
            <a class="sonara-quickstart-card sonara-quickstart-card--growth" href="/growth-studio/campaigns"><strong>Growth Studio</strong><span>Plan a campaign</span><small>Consent, leads, campaigns, and measurement.</small></a>
            <a class="sonara-quickstart-card sonara-quickstart-card--setup" href="/readiness"><strong>Shared setup</strong><span>Review setup</span><small>See what is configured and what still needs attention.</small></a>
          </div>
        </section>` : ""}
        <section class="grid">${renderedSections}</section>
      </main>
      <footer>
  ...
  // renderHomepageContent was removed on 8 September 2026: 108 lines of a
  // homepage body that nothing rendered.
  //
  // server.js destructures what it needs from createPageFrame and never took
  // this one; the live homepage is built inline in app.get("/"). The only other
  // mentions were a comment in lib/sonara-shell.cjs and the function list in
  // tests/server-split.test.js, which records where the split put code rather
  // than that anything calls it.
  //
  // scripts/report-unreferenced-modules.mjs could not see it: it works at module
  // granularity and this module is heavily referenced. Its own header describes
  // two homepage renderers that were noticed three separate times and left each
  // time, because noticing is free and deleting needs somebody to be sure. This
  // is the same thing one level down, and the same answer.
  ...
  function adminActions() {
    return [
      ...
      linkAction("/admin/agent-activity", "Agent control plane"),
