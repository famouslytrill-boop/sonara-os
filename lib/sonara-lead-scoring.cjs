...
function scoreRisk(profile = {}, answers = {}, activity = {}) {
  ...
  // A throwaway address, passed in as a boolean rather than as the address.
  //
  // lib/sonara-lead-capture-script.cjs deliberately keeps contact details out of
  // what reaches this module, and a test asserts it -- so the lookup happens
  // where the address already is, and only the answer travels. Worth few points:
  // it is a hint about intent, not a disqualifier, and a real customer whose
  // company mail is down uses one.
  if (activity.disposableEmail === true) {
    flags.push({ code: "disposable_email", detail: "the address is a throwaway one", points: 15 });
  }
