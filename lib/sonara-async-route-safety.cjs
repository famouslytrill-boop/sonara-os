...
const { redactError, redactSensitiveText } = require("./sonara-redaction.cjs");
...
function createAsyncErrorHandler(deps = {}) {
  ...
  return function sonaraAsyncErrorHandler(error, req, res, next) {
    ...
    // Redacted, and this is the sink that most needed it.
    //
    // Every unhandled route error in the application arrives here, and the line
    // used to interpolate `error.stack` raw. lib/sonara-redaction.cjs was
    // written for precisely this carrier -- its own header says "a Supabase
    // failure carries a URL with an apikey query parameter ... redactError()
    // exists because an Error is the usual carrier and `String(error)` drops the
    // stack while `error.stack` keeps the URL that failed" -- and this handler
    // did not call it. A stack from a failed PostgREST request prints the
    // service-role key, which is the single worst thing in this deployment to
    // print, on the path taken when something is already going wrong.
    //
    // The URL is scrubbed too, separately: a request path can carry a token in
    // its query string (a shared-result link is one), and that is a credential
    // in the log for the same reason.
    const where = redactSensitiveText(`${req.method} ${req.originalUrl || req.url}`);
    log(`[route-error] ${where} -> ${redactError(error)}`);
