...
const { getGenerationPathwayPlan } = require("./sonara-generation-pathway-planner.cjs");
...
const ENGINE_CATALOG = Object.freeze([
  ...
  engine({ key: "huggingface_diffusers", label: "Hugging Face Diffusers", kind: "generation_pipeline_library", repository: "huggingface/diffusers", license: "Apache-2.0", adoptionStatus: "commercial_candidate_with_model_review", runtimeBoundary: "isolated_gpu_worker", products: ["Creator Studio"], capabilities: ["image_generation", "image_editing", "video_generation", "audio_generation", "adapter_workflows"], restrictions: ["library license does not determine checkpoint, model-weight, dataset, or output rights", "run in an isolated worker rather than the hosted Express/Vercel request process"] }),
  ...
  engine({ key: "triposr", label: "TripoSR", kind: "image_to_3d_model_family", repository: "VAST-AI-Research/TripoSR", license: "MIT", adoptionStatus: "commercial_candidate_with_model_review", runtimeBoundary: "isolated_gpu_worker", products: ["Creator Studio"], capabilities: ["image_to_3d", "single_image_3d_reconstruction"], restrictions: ["repository license is permissive; weights, input-image rights, training-data provenance, and generated-asset use remain separately reviewable", "3D generation runs in an isolated worker, not the web request process"] }),
...
function getModelEngineControlPlane() {
  ...
  const generationPlan = getGenerationPathwayPlan();
  return {
    ok: true,
    ...
    generationCreationPathways: {
      mode: generationPlan.mode,
      modalityCount: generationPlan.modalities.length,
      pathwayCount: generationPlan.pathways.length,
      alternativeCount: generationPlan.alternatives.length,
      modalities: [...generationPlan.modalities],
      pathways: generationPlan.pathways.map((entry) => ({
        key: entry.key,
        label: entry.label,
        runtimeLane: entry.runtimeLane,
      ...
      alternatives: generationPlan.alternatives.map((entry) => ({
        key: entry.key,
        label: entry.label,
        pathway: entry.pathway,
        status: entry.status,
        enabledByPlanner: false,
        canExecuteFromPlanner: false
      })),
      routingPrinciples: [...generationPlan.routingPrinciples],
      boundaries: [...generationPlan.boundaries]
    },
    openSource,
    boundaries: [
      ...
      "The generation/creation pathway planner adds deterministic, browser-native, hosted, marketplace, managed-worker, self-hosted, owner-device, companion and hybrid routes without silently executing or switching providers.",
