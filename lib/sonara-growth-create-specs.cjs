...
const GROWTH_CREATE_SPECS = Object.freeze([
  ...
  },
  {
    ...
    fields: [
      ...
      // Labels, because the renderer's fallback is the key with its underscores
      // swapped for spaces -- which offered "send webhook" and "sync provider"
      // to somebody choosing what an automation should do.
      ["action_key", "choice", {
        label: "Do this",
        values: ["create_task", "notify_owner", "add_to_segment", "enqueue_email", "sync_provider", "send_webhook"],
        labels: {
          create_task: "Create a task",
          notify_owner: "Tell the owner",
          add_to_segment: "Add to a group",
          enqueue_email: "Queue an email",
          sync_provider: "Send to a connected service",
          send_webhook: "Notify another system"
        },
  ...
  {
    // Offline touchpoints, refused twice and now built, because the thing that
    // was missing both times exists.
    //
    // The note that stood here said: "A typed-in touchpoint is indistinguishable
    // from a tracked one, and it feeds the trail every attribution figure is
    // matched against... If offline touchpoints are wanted later, the honest
    // version starts with a column recording that a person entered it."
    //
    // Migration 20260818090000 adds `hand_entered`, and the two halves that make
    // this safe landed with it rather than after it:
    //
    //   * the endpoint records true for anything submitted here, and null --
    //     not false -- when a caller says nothing;
    //   * the "Reached" stage of lib/sonara-customer-journey.cjs counts only
    //     rows that are not hand-entered, and reports the typed ones separately.
    //
    // So a business can record the conversation at the counter, see it, and
    // still read a funnel that is measured. Without the second half this form
    // would let somebody raise their own reach by typing, which is what the two
    // earlier refusals were protecting against.
    //
    // No provider_key, anonymous_id or external_event_id fields: those identify
    // a tracked source, and offering them on a hand-entry form is offering to
    // dress a typed row as a measured one.
    key: "touchpoints",
    tableKey: "touchpoints",
    noun: "contact",
    intro: "Something that happened away from any tracking -- a conversation at the counter, a call, somebody mentioning where they heard of you. It is recorded as entered by hand, kept out of your measured reach, and shown separately, so your funnel stays honest.",
    fields: [
      ["event_name", "text", { label: "What happened", max: 200, required: true, hint: "Mentioned us at the counter, called about the advert, met at the market" }],
      ["channel", "text", { label: "How it reached them", max: 100, hint: "In person, phone, word of mouth" }],
      ["source", "text", { label: "Where it came from", max: 200, hint: "A neighbour, the local paper, a repeat customer" }],
      ["occurred_at", "text", { label: "When it happened", max: 40, hint: "Leave blank for now" }],
      // The handler refuses to write without this, exactly as the content
      // handler does. A form missing the box would send every submission into a
      // 400 the customer cannot act on -- which is the defect
      // tests/growth-record-pages-paid.test.js already guards for content.
      ["tracking_basis_attested", "checkbox", { label: "I have a lawful basis for recording this contact, and the person knows who I am" }]
    ]
  }
]);
