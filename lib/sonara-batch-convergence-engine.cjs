...
const { getScreenshotToolReadinessBatch11 } = require("./sonara-screenshot-tool-radar-batch11.cjs");
...
const EXTERNAL_BATCHES = Object.freeze([
  ...
  // Batch 11, the 16 September 2026 intake. Numbered 11 because 8 and 9 are the
  // capability and design convergence records and 10 is the operational review
  // required just below -- the screenshot-radar filename sequence stopping at 7
  // is what makes 8 look free. An intake recorded in the radar but missing from
  // this list would be published on the Research Lab page and absent from every
  // convergence figure, which is the same defect one level up: a record that
  // looks accounted for because it exists somewhere.
  Object.freeze({ batch: 11, getter: getScreenshotToolReadinessBatch11 })
]);
...
function getUnifiedBatchConvergence() {
  ...
  return {
    ok: true,
    mode: "batch_1_10_plus_formal_registry_governed_convergence",
    batchCount: 10,
...
function normalizeBatch10Repository(record = {}) {
  return {
    ...
    commercialUseStatus: /NOASSERTION/i.test(String(record.license)) ? "blocked_until_review" : "needs_review",
...
function numericBatches(value) {
  return Number.isInteger(value) && value >= 1 && value <= 10 ? [value] : [];
}
