...
const { getScreenshotToolReadinessBatch14 } = require("./sonara-screenshot-tool-radar-batch14.cjs");
const { getScreenshotToolReadinessBatch15 } = require("./sonara-screenshot-tool-radar-batch15.cjs");
const { getScreenshotToolReadinessBatch16 } = require("./sonara-screenshot-tool-radar-batch16.cjs");
const { getScreenshotToolReadinessBatch17 } = require("./sonara-screenshot-tool-radar-batch17.cjs");
...
const EXTERNAL_BATCHES = Object.freeze([
  ...
  // Batch 14, the 19 September 2026 browser/security/voice/business-tool intake.
  // It is research-only and does not widen runtime authority.
  Object.freeze({ batch: 14, getter: getScreenshotToolReadinessBatch14 }),
  // Batch 15, the remaining 19 September 2026 system-design, agent, data, media, gateway, and reliability intake.
  Object.freeze({ batch: 15, getter: getScreenshotToolReadinessBatch15 }),
  // Batch 16, the 20 September 2026 developer-agent, MCP, browser, schema, capture,
  // visualization, market/media, ARM64 and Physical AI intake.
  Object.freeze({ batch: 16, getter: getScreenshotToolReadinessBatch16 }),
  // Batch 17, the 22 September 2026 decision-model, developer-tool, design,
  // finance, memory, mobile/network, and workflow intake.
  Object.freeze({ batch: 17, getter: getScreenshotToolReadinessBatch17 })
]);
...
function getUnifiedBatchConvergence() {
  ...
  return {
    ok: true,
    mode: "batch_1_16_plus_formal_registry_governed_convergence",
...
function numericBatches(value) {
  return Number.isInteger(value) && value >= 1 && value <= 14 ? [value] : [];
}
