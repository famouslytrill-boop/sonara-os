...
const CAPABILITY_EXPANSION_TRACKS = Object.freeze([
  ...
  capabilityTrack({
    key: "commerce_store_operations",
    label: "Unified commerce and store operations",
    status: "next_build",
    phase: 1,
    customerValue: "Sell the same canonical products through owned storefronts, POS, kiosks, pickup, delivery, and physical stores while keeping inventory, customers, orders, returns, and payment references synchronized.",
    businessValue: "Turns Business Builder into an owned-commerce operating layer for online sellers and brick-and-mortar operators without creating separate databases for each sales channel.",
    target: "Build one tenant-scoped commerce contract over existing customer, catalog, inventory, vendor, location, order, invoice, payment, workflow, audit, and analytics primitives. Prove product/variant/SKU/GTIN identity, stock reservations, purchase/receiving, storefront checkout, POS/kiosk order capture, provider-confirmed payments, returns/refunds, register reconciliation, BOPIS, ship-from-store, and own-brand product lifecycle before expanding channels.",
    technologies: ["canonical commerce model", "POS", "kiosk", "PWA storefront", "Stripe Terminal evaluation", "GS1/GTIN", "inventory reservations", "purchase orders", "fulfillment", "returns", "Google Merchant adapters"],
    proofGates: [
      "one product/variant identity across storefront and POS",
      "tenant and location isolation",
      "concurrency-safe stock reservation",
      "idempotent order payment and inventory mutations",
      "provider-confirmed in-person and online payment states",
      "register and tender closeout reconciliation",
      "BOPIS happy cancellation and expiry paths",
      "cross-channel return and refund reconciliation",
      "offline and provider-degraded reconciliation",
      "barcode and GTIN validation without fabricated identifiers",
      "tax configuration or provider proof",
      "accessible storefront POS and kiosk paths",
      "one-tenant one-location canary with rollback"
    ],
    claimBoundary: "Research, UI, formulas, provider documentation, and open-source catalogs do not prove unified commerce. SONARA may claim only the channels, hardware, payment paths, inventory synchronization, and omnichannel workflows that have passed tenant, concurrency, provider, accessibility, reconciliation, canary, and production evidence.",
    productionEnabled: false,
    evidenceUrls: [
