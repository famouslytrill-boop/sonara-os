...
// ## What this is not
//
// It is not a logging framework and it adds no dependency -- that was the
// point when this was written against a single production dependency, and it is
// still the point now there are nine. docs/architecture/EXTERNAL-SERVICES.md
// sets the rules a new one has to clear; the eight @opentelemetry packages and
// @openfeature/server-sdk that arrived on 20 September did not clear them
// through this module, and nothing in the runtime calls either of the two
// modules that consume them. docs/SHIP_READINESS.md has the measurement.
//
