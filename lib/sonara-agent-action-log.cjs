...
function createActionHistoryReader({ organizationId, agentKey, getSupabaseServerConfig, window = 10 }) {
  return async function readRecentOutcomes() {
    ...
    const query =
      ...
      `&order=created_at.desc&limit=${Number(window) || 10}`;
    ...
    return { ok: true, rows: body.map((row) => ({ status: String(row?.result || "") })) };
  };
}

module.exports = { TABLE, toRow, riskLevelFor, approvalStateFor, createActionLogRecorder, createActionHistoryReader };
