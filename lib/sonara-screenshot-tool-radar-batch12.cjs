...
// Batch 12 screenshot intake, 16 September 2026. Evidence and planning data
...
// from `evidence`, and `getScreenshotToolReadinessBatch12` refuses to count a
...
const NON_REPOSITORY_REFERENCES_BATCH12 = Object.freeze([
...
const SCREENSHOT_TOOL_RADAR_BATCH12 = Object.freeze(REPOSITORIES.map((input) => Object.freeze({
  ...input,
  ...
  source: "user_submitted_screenshot_research_batch12_2026_09_16"
})));

function getPublicScreenshotToolCatalogBatch12() {
  return SCREENSHOT_TOOL_RADAR_BATCH12.map((item) => ({
    ...item,
...
function getNonRepositoryReferencesBatch12() {
  return NON_REPOSITORY_REFERENCES_BATCH12.map((item) => ({ ...item, safety: [...item.safety] }));
}

function getConductRefusalsBatch12() {
  return CONDUCT_REFUSALS.map((item) => ({ ...item }));
}

function getConfirmedExistingRecordsBatch12() {
  return CONFIRMED_EXISTING_RECORDS.map((item) => ({ ...item }));
}

function getScreenshotToolReadinessBatch12() {
  const repositories = getPublicScreenshotToolCatalogBatch12().map((item) => ({
    ...item,
  ...
  return {
    ok: true,
    mode: "static_governed_screenshot_research_batch12",
    ...
    nonRepositoryReferenceCount: NON_REPOSITORY_REFERENCES_BATCH12.length,
    ...
    repositories,
    nonRepositoryReferences: getNonRepositoryReferencesBatch12(),
