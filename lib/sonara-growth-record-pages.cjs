...
const { finiteNumber } = require("./sonara-owner-record-pages.cjs");
...
// Same fault as the Business Builder helpers, in a second copy. Number(null) is
// 0 and 0 is finite, so a segment nobody had evaluated reported "0" people
// rather than "not counted", and an unmeasured rate read as 0%. A stored zero
// still renders as zero; something wrote it.
function countText(value) {
  const parsed = finiteNumber(value);
  return parsed === null ? "Not counted" : String(parsed);
}

function percentText(value) {
  const parsed = finiteNumber(value);
  return parsed === null ? "Not set" : `${Math.round(parsed * 100)}%`;
}
...
function conversionRate(row) {
  const exposures = finiteNumber(row.exposures);
  const conversions = finiteNumber(row.conversions);
  if (exposures === null || conversions === null || exposures <= 0) return "Not enough data";
...
const GROWTH_RECORD_PAGES = Object.freeze([
  {
    ...
    // "Nothing is sent from here" was true when this page was written and is
    // not any more. The route exists, so the sentence had to change or become
    // the kind of stale copy that tells a customer a capability is absent while
    // it works -- the same defect as the paid placeholder pages above, one
    // direction reversed.
    body: "The efforts you have under way to bring people in, and where each one stands. You can email one from here, to the people who recorded consent for it.",
    select: "id,name,goal,channel,status,created_at",
    // Renders the send form and the outcome of the last send. Only this page
    // sets it: sending is a campaign action and the other nine pages list other
    // things.
    sendForm: true,
  ...
  },
  {
    ...
    columns: [
      ...
      { label: "Status", value: (row) => growthStatusLabel(row.status) },
      // `Number(row.progress_percent || 0)%` printed "0%" for a goal nobody has
      // measured yet, which reads as "no progress" rather than "not recorded".
      // Same shape as the menu page reporting a 100% margin on an uncosted
      // dish: a confident number standing in for an absent one.
      { label: "Progress", value: (row) => (finiteNumber(row.progress_percent) === null ? "Not measured" : `${finiteNumber(row.progress_percent)}%`) },
