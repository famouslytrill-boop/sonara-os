...
// that would change both cost and the processor receiving the prompt. The
// default mode remains `local_rules`, which never makes a network call.
...
function normalizeProvider(value) {
  return String(value || LOCAL_PROVIDER).trim().toLowerCase();
}
...
async function generate({ provider, messages, maxTokens } = {}, options = {}) {
  ...
  if (selected === openai.PROVIDER) {
    return openai.generate(messages, { ...options, maxTokens });
  }
