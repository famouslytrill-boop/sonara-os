...
const STRIPE_PLANS = {
  ...
  // Canonical paid ladder.
  //
  // The retired depth ladder was removed from runtime on 18 September 2026
  // after live Stripe verification proved its three exact prices had zero
  // subscriptions. Those price objects and their products are archived in
  // Stripe. Runtime authorization, checkout, limits and readiness now recognize
  // only this workspace-based ladder plus the quoted setup package below.
  //
  // A Stripe price is immutable. Changing one of these advertised amounts
  // therefore requires a new Stripe Price and a matching canonical environment
  // variable. Checkout verifies the live Stripe amount and product activity
...
//
// Resolve this from the plans actually being sold so pricing prose cannot
// outlive the current catalogue.
...
// sentences used to be written out by hand. Prose that names a plan can
// outlive the plan; prose derived from the table cannot.
