...
const DATABASE_TABLE_GROUPS = Object.freeze({
  ...
  billing: Object.freeze([
    ...
    // The append-only credit ledger for metered capabilities. Billing rather
    // than agentsAndAutomation: it is money in and money out, and it is what
    // lets lib/sonara-paid-capabilities.cjs charge for a GPU second, a carrier
    // minute or an SMS instead of pricing them and billing nobody.
    "usage_credit_ledger"
  ]),
...
const DATABASE_FUNCTIONS = Object.freeze([
  ...
  "public.create_sonara_prompt_version(uuid,text,text,text,text)",
  "public.sonara_database_contract_snapshot()"
]);

// This foundation postdates the frozen canonical inventory. Keep it separate
// so readiness and production verification cover it without silently changing
// the historical contract count or pretending a broker/worker now exists.
const DURABLE_EVENT_FOUNDATION_TABLES = Object.freeze([
  "event_outbox",
...
const DURABLE_EVENT_FOUNDATION_FUNCTIONS = Object.freeze([
  "public.claim_sonara_event_outbox(uuid,text,timestamptz)",
  "public.claim_sonara_event_outbox_filtered(uuid,text,text[],text[],timestamptz)",
  "public.settle_sonara_event_outbox(uuid,uuid,text,text,text,timestamptz,timestamptz)"
]);

// Tenant-scoped, human-editable language records are reviewed separately from
// the frozen canonical inventory until the application editor and runtime
// reader ship. They are still part of the database contract and receive
// explicit RLS and role-grant checks.
const TRANSLATION_FOUNDATION_TABLES = Object.freeze([
  "translation_records",
  "translation_glossary_terms"
]);

const DURABLE_WORKER_FUNCTIONS = Object.freeze([
