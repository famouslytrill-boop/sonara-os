...
// Annual plans are not listed here. `withAnnualTwins` expands every list below
// from lib/sonara-stripe-plans.cjs, so a plan's annual form opens exactly what
// its monthly form opens and cannot drift from it by an omission.
const { withAnnualTwins } = require("./sonara-stripe-plans.cjs");
...
//
// The active entitlement model is the canonical breadth ladder only.
// Live Stripe verification on 2026-09-18 found zero subscriptions on the retired
// Starter/Core/Pro prices, so those compatibility keys no longer participate in
// authorization. workspace_monthly buys exactly one chosen workspace;
// all_three_monthly and team_monthly open all three, with Team also opening the
// staff portal. Annual twins are derived below.
// The monthly plans, written out. Annual plans are added below rather than
// listed here, and the split is not cosmetic.
//
// PAID_ACCESS_RUNTIME_MARKERS renders these three lines back into the exact
// text they have in this file, and the production deploy gate greps the shipped
// runtime for that text as proof the fail-closed path actually deployed. A
// marker is only proof while it is something the source really says: expanding
// the lists in place made the rendered marker text appear nowhere, and the gate
// went looking for a string that no longer existed. So the literals stay
// literal, and the expansion happens to a copy.
const BASE_ENTITLEMENT_KEYS = Object.freeze({
  business_builder: ["business_builder_one_time", "workspace_monthly", "all_three_monthly", "team_monthly"],
  creator_studio: ["workspace_monthly", "all_three_monthly", "team_monthly"],
  growth_studio: ["workspace_monthly", "all_three_monthly", "team_monthly"]
});

const PAID_ENTITLEMENT_KEYS = Object.freeze({
  business_builder: withAnnualTwins(BASE_ENTITLEMENT_KEYS.business_builder),
  creator_studio: withAnnualTwins(BASE_ENTITLEMENT_KEYS.creator_studio),
  growth_studio: withAnnualTwins(BASE_ENTITLEMENT_KEYS.growth_studio)
});
...
const SINGLE_WORKSPACE_PLANS = Object.freeze(withAnnualTwins(["workspace_monthly"]));
...
const FEATURE_ENTITLEMENT_KEYS = Object.freeze({
  staff_portal: withAnnualTwins(["team_monthly"])
});
...
const PLAN_FLOOR_ENTITLEMENT_KEY = Object.freeze({
  starter: "workspace_monthly",
  core: "all_three_monthly",
  pro: "team_monthly"
});

// Whether the catalogue floor a product advertises maps to an entitlement
// that actually opens it. Floor names describe product depth; the mapping above
// resolves those labels to the canonical workspace billing ladder so catalogue
// copy and server authorization cannot drift into separate plan systems.
...
const PAID_ACCESS_RUNTIME_MARKERS = Object.freeze([
  ...
  ...Object.entries(BASE_ENTITLEMENT_KEYS).map(
    ([productKey, keys]) => `${productKey}: [${keys.map((key) => `"${key}"`).join(", ")}]`
  ),
  // The expansion itself, so the gate proves the annual plans reached the
  // deployed runtime too. Without this the three lines above would ship and an
  // annual subscriber would be refused by a bundle that passed every marker.
  "withAnnualTwins(BASE_ENTITLEMENT_KEYS.business_builder)",
