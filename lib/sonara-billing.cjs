...
function createBilling(deps = {}) {
  ...
  async function getOrCreateStripeCustomer(user, organizationId) {
    ...
    // Storing the mapping, and saying so when it did not store.
    //
    // This write was `.catch(() => undefined)` with the result discarded, and
    // the consequence of losing it is not a missing row -- it is a **second
    // Stripe customer**. The lookup above is what stops one being created; if
    // the mapping is absent the next checkout misses, creates another Stripe
    // customer for the same person, and the two accumulate silently with a
    // subscription potentially on each.
    //
    // The checkout still proceeds and this still returns ok, deliberately: the
    // customer Stripe just created is real and usable for this session, and
    // refusing here would turn a bookkeeping failure into a lost sale. What
    // changes is that it is no longer invisible.
    ...
    if (!stored?.ok) {
      emitEvent({
        ...
        detail: {
          ...
          consequence: "the Stripe customer exists and is not mapped locally, so the next checkout for this user will create a second one"
        }
      });
    }
  ...
  async function synchronizeBillingFromStripeEvent(event) {
    ...
    const planSlug = String(subscription?.metadata?.plan || "").trim();
    if (!config.ok || !subscription?.id || !organizationId) return { ok: false };
    if (!planSlug || STRIPE_PLANS[planSlug]?.mode !== "subscription") {
    ...
    // When Stripe stamped this event, not when it reached us. Stripe does not
    // guarantee delivery order and retries on any non-2xx or timeout, so the
    // order these arrive in says nothing about the order they happened in.
    //
    // The database discards a write carrying an older stamp than the row holds
    // -- see 20260903120000_stripe_events_cannot_arrive_backwards.sql for why
    // the comparison is there and not here. Null when Stripe sends no `created`,
    // which the trigger treats as "apply", so a missing stamp cannot silently
    // turn a real update into a no-op.
    const providerEventAt = Number.isFinite(event.created) ? new Date(event.created * 1000).toISOString() : null;
  ...
  async function getBillingPanelSummary(organizationId) {
    ...
    if (!response?.ok) return { ok: false, status: "We could not check your plan just now. Try again shortly.", rows: [] };
    const rows = await response.json().catch(() => []);
    const active = rows.find((row) => ["active", "trialing"].includes(row.status));
    return {
      ok: true,
      status: active ? `${displayStatus(active.plan_slug)}: ${displayStatus(active.status)}${periodSentence(active)}` : "No active paid plan found.",
      rows
    };
  }
