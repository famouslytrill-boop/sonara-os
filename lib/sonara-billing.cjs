...
function createBilling(deps = {}) {
  ...
  async function assertPriceMatchesAdvertised(plan, priceId) {
    ...
    // it belongs to is archived, and that is not a hypothetical shape: the
    // three retired plans on this account are exactly it. Archiving a product
    // in Stripe does not flip its prices' active flag, so all three read
