...
// Batch 10 screenshot intake, 16 September 2026. Evidence and planning data
...
// from `evidence`, and `getScreenshotToolReadinessBatch10` refuses to count a
...
const NON_REPOSITORY_REFERENCES_BATCH10 = Object.freeze([
...
const SCREENSHOT_TOOL_RADAR_BATCH10 = Object.freeze(REPOSITORIES.map((input) => Object.freeze({
  ...input,
  ...
  source: "user_submitted_screenshot_research_batch10_2026_09_16"
})));

function getPublicScreenshotToolCatalogBatch10() {
  return SCREENSHOT_TOOL_RADAR_BATCH10.map((item) => ({
    ...item,
...
function getNonRepositoryReferencesBatch10() {
  return NON_REPOSITORY_REFERENCES_BATCH10.map((item) => ({ ...item, safety: [...item.safety] }));
}

function getConductRefusalsBatch10() {
  return CONDUCT_REFUSALS.map((item) => ({ ...item }));
}

function getConfirmedExistingRecordsBatch10() {
  return CONFIRMED_EXISTING_RECORDS.map((item) => ({ ...item }));
}

function getScreenshotToolReadinessBatch10() {
  const repositories = getPublicScreenshotToolCatalogBatch10().map((item) => ({
    ...item,
  ...
  return {
    ok: true,
    mode: "static_governed_screenshot_research_batch10",
    ...
    nonRepositoryReferenceCount: NON_REPOSITORY_REFERENCES_BATCH10.length,
    ...
    repositories,
    nonRepositoryReferences: getNonRepositoryReferencesBatch10(),
...
module.exports = {
  SCREENSHOT_TOOL_RADAR_BATCH10,
  NON_REPOSITORY_REFERENCES_BATCH10,
  CONDUCT_REFUSALS,
