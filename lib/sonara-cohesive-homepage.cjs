...
function statusItem(label, value, detail) {
  ...
  return `<li class="sonara-cohesive-status is-${statusTone(status)}"><span aria-hidden="true"></span><div><strong>${escapeHtml(label)}</strong><small><b>${escapeHtml(statusLabel(status))}</b>${escapeHtml(detailText)}</small></div></li>`;
}
...
function renderCohesiveHomepage(readiness = {}) {
  ...
  return `<div class="sonara-cohesive-system" data-sonara-cohesive data-sonara-product-current="${escapeHtml(business.slug)}">
    ...
    <section class="sonara-cohesive-intro" aria-labelledby="sonara-cohesive-intro-title">
      <div>
        ...
        <h2 id="sonara-cohesive-intro-title">Build what matters. Keep it moving.</h2>
