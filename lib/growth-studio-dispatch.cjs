...
// One recipient per HTTP request, and this is a choice rather than a vendor
// limit.
//
// CORRECTED 10 September 2026. This constant's comment previously said 1 was
// "Resend's own documented ceiling for a batch call". It is not:
// resend.com/docs/api-reference/emails/send-batch-emails, read on that date,
// says a batch call takes "up to 100 batch emails at once". The number was
// right and the reason was invented, which is exactly the defect CLAUDE.md
// names -- a reason reasoned into rather than verified reads identically to one
// that was checked, and it is what the next person reads instead of checking.
//
// The real reason for 1 is attribution. `failed` carries an address and a
// status per recipient, and a 100-recipient batch that partly fails reports at
// the batch level -- so an owner would be told 100 went out with no way to say
// which 3 did not. Batching is the obvious optimisation the moment a campaign
// is large enough to need it, and it costs that attribution.
...
async function dispatchCampaign({
  ...
  for (const recipient of decision.eligible) {
    // Per recipient, because the token names them. One shared link would let
    // any recipient unsubscribe every other -- and would make a forwarded email
    // a way to remove somebody else from the list.
    const token = signToken({ organizationId, leadId: recipient.id, key: signingKey });
    const url = unsubscribeUrl({ origin, token });
    const body = withUnsubscribeLine(cleanBody, url);
    const headers = unsubscribeHeaders(url);

    if (!token || !url || !body || !headers) {
    ...
    const response = await fetchImpl(RESEND_ENDPOINT, {
      ...
      body: JSON.stringify({ from, to: [recipient.email], subject: cleanSubject, text: body, headers })
    }).catch(() => undefined);
