...
// are still unbuilt. Until then the summary says what is true rather than what
// would be convenient.
...
async function dispatchCampaign({
  ...
  if (accepted.length > 0 || failed.length > 0 || notAttempted.length > 0) {
    ...
    if (!built.ok) {
      report({ code: built.code, detail: `campaign ${campaignId} sent ${accepted.length} and no send record could be built: ${built.detail}` });
    ...
    if (built.ok) {
      if (typeof recordSends !== "function") {
        ...
        report({ code: "send_record_not_wired", detail: `campaign ${campaignId} sent ${accepted.length} with no send-record writer supplied, so the remainder cannot be worked out` });
