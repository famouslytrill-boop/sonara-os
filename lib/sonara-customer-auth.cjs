...
const GOOGLE_OAUTH_VERIFIER_COOKIE = "sonara_google_oauth_verifier";
const GOOGLE_OAUTH_NEXT_COOKIE = "sonara_google_oauth_next";
...
function createCustomerAuth(deps = {}) {
  ...
  async function beginGoogleOAuth(req, res, nextPath = "/dashboard") {
    ...
    res.cookie(GOOGLE_OAUTH_VERIFIER_COOKIE, verifier, googleOAuthCookieOptions());
    res.cookie(GOOGLE_OAUTH_NEXT_COOKIE, normalizeOAuthNextPath(nextPath), googleOAuthCookieOptions());

    return { ok: true, status: 303, url: authorizeUrl.toString() };
  }
  ...
  async function completeGoogleOAuth(req, res) {
    ...
    const verifier = getCookie(req, GOOGLE_OAUTH_VERIFIER_COOKIE);
    const nextPath = normalizeOAuthNextPath(getCookie(req, GOOGLE_OAUTH_NEXT_COOKIE));
    ...
    setCustomerSessionCookies(res, { accessToken, refreshToken, maxAgeSeconds });
    return { ok: true, status: 303, code: "google_login_ready", nextPath };
  }
