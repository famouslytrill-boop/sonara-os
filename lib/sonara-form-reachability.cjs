...
function recordPageFormActions() {
  ...
  for (const value of Object.values(pages)) {
    ...
    for (const page of value) {
      ...
      // A record with line items renders a second form, on its detail page, at
      // page.lines.api. Reading only page.api reported all four line endpoints
      // as having no form while the form was on screen -- the same shape of
      // miss as reading only the first exported array did for
      // CREATOR_RECORD_PAGES.
      if (page.lines && typeof page.lines.api === "string" && page.lines.api.startsWith("/")) found.add(page.lines.api);
    }
  }
  return found;
}
