...
const ENGINE_VERSION = "2026-07-26.3";
...
const BLOCKED_TRIGGER_TERMS = Object.freeze({
  "nearby-presence": Object.freeze(["nearby friends", "live location", "presence tracking", "continuous location", "real time location sharing"]),
  "digital-wallet": Object.freeze(["digital wallet", "stored value", "wallet balance", "hold customer funds", "money transfer", "peer to peer transfer", "stored customer balances"]),
  "stock-exchange": Object.freeze(["stock exchange", "matching engine", "order book", "securities", "brokerage", "trading", "market settlement"])
});
...
function reviewSystemDesign(input = {}) {
  ...
  const normalizedSearchText = normalizePhrase(searchableText);
  ...
  const explicitBlockedSlugs = Object.keys(BLOCKED_TRIGGER_TERMS)
    .filter((slug) => isExplicitlyBlocked(slug, normalizedSearchText));
  ...
  for (const blockedSlug of explicitBlockedSlugs) {
    if (selectedSlugs.has(blockedSlug)) continue;
    const blockedPattern = getSystemDesignPattern(blockedSlug);
    if (blockedPattern) {
      selected.push({
        pattern: blockedPattern,
        score: 100,
        matchedTerms: matchingBlockedTriggers(blockedSlug, normalizedSearchText)
      });
      selectedSlugs.add(blockedSlug);
    }
  }
  ...
  for (const baselineSlug of BASELINE_PATTERN_SLUGS) {
    ...
    if (baseline) {
      selected.push({ pattern: baseline, score: 1, matchedTerms: ["baseline architecture gate"] });
      selectedSlugs.add(baselineSlug);
    }
  }

  selected.sort((a, b) => b.score - a.score || a.pattern.order - b.pattern.order);

  const blockedMatches = selected.filter((item) => explicitBlockedSlugs.includes(item.pattern.slug));
...
function isExplicitlyBlocked(slug, normalizedSearchText) {
  return matchingBlockedTriggers(slug, normalizedSearchText).length > 0;
}

function matchingBlockedTriggers(slug, normalizedSearchText) {
  const triggers = BLOCKED_TRIGGER_TERMS[slug] || [];
  return triggers.filter((trigger) => normalizedSearchText.includes(normalizePhrase(trigger)));
}
