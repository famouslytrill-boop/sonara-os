...
const ENGINE_VERSION = "2026-07-26.2";
...
const BLOCKED_TRIGGER_TERMS = Object.freeze({
  ...
  "digital-wallet": Object.freeze(["digital wallet", "stored value", "wallet balance", "hold customer funds", "money transfer", "peer to peer transfer"]),
...
function reviewSystemDesign(input = {}) {
  ...
  const blockedMatches = selected.filter((item) => isExplicitlyBlocked(item.pattern.slug, searchableText));
...
function isExplicitlyBlocked(slug, searchableText) {
  const triggers = BLOCKED_TRIGGER_TERMS[slug] || [];
  return triggers.some((trigger) => searchableText.includes(trigger));
}
