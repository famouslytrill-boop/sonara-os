...
// margin line: `quote()` already returns `marginMinor`.
//
// No figure is quoted here on purpose. This said "telephony prices at 3 minor
// units against a 0.8 floor" and went stale the moment that floor was corrected
// to a sourced 1.4 -- a copied number in a second file is a number that will
// disagree with the first one eventually. `lib/sonara-paid-capabilities.cjs`
// holds them all, each with its source and the date it was read.
//
...
//
// 500 minor units is $5.00. At media generation's current price that buys about
// two thousand GPU seconds -- roughly twenty eight-second videos on the current
// estimate. The price itself is not copied here:
// `tests/a-cost-floor-is-a-figure-somebody-checked.test.js` checks that this
// allowance still buys that order of magnitude, so a price change fails the claim
// instead of quietly making it wrong.
//
// A figure to be replaced by a per-plan entitlement; recorded here so it is one
// number in one place until then.
