 ...
 *
 * `customersRead` says whether the customer list was actually readable, and it
 * is not decoration. An empty `customersById` has two causes that look
 * identical here -- the business genuinely has no customers, or the table could
 * not be read -- and only one of them is a fact about the invoice. Without this
 * flag every overdue invoice is reported as "not attached to a customer", which
 * is false, specific, and comes with an instruction the owner would go and act
 * on. Same reasoning, and the same shape, as `paymentsRead` in
 * `lib/sonara-invoice-settlement.cjs`.
 */
function build({ invoices = [], customersById = new Map(), paidByInvoice = new Map(), businessName = "", now = Date.now(), customersRead = true } = {}) {
  const drafts = [];
  const skipped = [];
  ...
  for (const invoice of Array.isArray(invoices) ? invoices : []) {
    ...
    if (!customer || !String(customer.name || "").trim()) {
      skipped.push({
        id: invoice.id,
        reference: reference || "an invoice with no number",
        // Three states, not two. "It is not attached to a customer" is a claim
        // about this invoice, and it is only knowable when the customer list
        // was readable. When it was not, nobody can tell -- and telling an
        // owner to set a customer on an invoice that already has one is work
        // they would actually go and do, on every overdue invoice at once.
        reason: customersRead
          ? "It is not attached to a customer, so there is nobody to address. Set the customer on the invoice."
          : "Your customer list could not be read, so there is no way to look up who this is for. There is probably nothing wrong with the invoice; try again shortly.",
        // The two are told apart by more than their wording, so a page can
        // group them without matching on prose.
        cause: customersRead ? "invoice" : "unreadable"
      });
      continue;
    }
