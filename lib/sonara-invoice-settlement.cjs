...
//
// ## Why there is still no pay button on the shared invoice
//
// **Corrected 6 September 2026.** This comment used to give two reasons, and
// the first of them stopped being true on 26 August. It said:
//
//   > There is no Stripe Connect in this application. No connected-account
//   > model, no `on_behalf_of`, no `transfer_data`, no table holding a
//   > business's Stripe account.
//
// There is now. `lib/sonara-connected-payments.cjs`,
// `routes/sonara-connected-payment-routes.cjs` and
// `20260826090000_business_payment_accounts.sql` build it, and
// `tests/a-business-can-be-paid-by-its-own-customers.test.js` covers it. A
// business can be paid by its own customers. The money-custody objection was
// answered rather than worked around: charges are created **on the connected
// account** with the `Stripe-Account` header, so funds land in the business's
// own Stripe balance and never enter SONARA's at any point.
//
// Leaving that paragraph here would have been worse than leaving no reason at
// all, because it is what the next person reads instead of checking -- which is
// the exact defect `CLAUDE.md` names as an exemption whose reason has expired.
//
// **The second reason still stands, and it is the whole reason this stays.**
// The shared invoice's footnote tells its reader to pay the way they agreed
// with the business and never from a link, because a forwarded invoice with a
// pay button is exactly the shape of a payment-redirection fraud. That advice
// only works if it is always true: a product where some shared invoices have a
// pay button and some do not has taught its customers that a pay button on an
// invoice link is normal, which is the lesson the fraud depends on.
//
// So payment is initiated by the business from inside the product, and
// `/shared/:token` keeps showing a balance and no button. This module computes
// that balance -- which the payment path needs underneath it either way.
