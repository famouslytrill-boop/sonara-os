...
function shiftSpans(shifts) {
  ...
  for (const shift of Array.isArray(shifts) ? shifts : []) {
    ...
    spans.push({ start, end, employeeId });
  }
  return spans.sort((a, b) => a.start - b.start);
}
 ...
 */
function freeStaffFor(start, end, { shifts, bookings }) {
...
function availableSlots({
  ...
  const shifts = staffed ? shiftSpans(staffShifts) : [];
  if (staffed && !shifts.length) {
    ...
    return {
      ok: true,
      reason: "Nobody is on the rota for this period yet, so there is no one to book with.",
      days: [], slots: 0, staffed: true, rosteredPeople: 0,
  ...
  for (let offset = 0; offset <= horizonDays + 1; offset += 1) {
    ...
    for (const range of ranges) {
      for (let minute = range.open; minute + duration <= range.close; minute += slotMinutes) {
        ...
        let free = null;
        if (staffed) {
          free = freeStaffFor(start, end, { shifts, bookings: spans });
  ...
  if (!days.length) {
    return {
      ...
      days: [],
      slots: 0,
      staffed, rosteredPeople,
      timeZone, slotMinutes, leadHours, horizonDays, truncated: false
    };
  }

  return { ok: true, reason: null, days, slots, staffed, rosteredPeople, timeZone, slotMinutes, leadHours, horizonDays, truncated };
}
 ...
 */
function isBookable({ page = {}, durationMinutes, bookings = [], staffShifts = [], startsAt, now = Date.now() } = {}) {
  const start = Date.parse(startsAt || "");
  if (!Number.isFinite(start)) return { ok: false, reason: "That is not a time." };

  const result = availableSlots({ page, durationMinutes, bookings, staffShifts, now });
