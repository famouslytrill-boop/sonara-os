...
// `Number(null)` is 0, and so are `Number("")` and `Number([])`. Reading an
// hour or a weekday with a bare `Number()` therefore turns "not recorded" into
// midnight, and into Sunday -- and a schedule whose hour reads as 0 is due at
// every hour, because `parts.hour < 0` is never true. It would fire on the next
// tick, whatever the time.
//
// No path in this repository produces such a row: `agent_schedules.hour_of_day`
// is `not null default 9`, and the one insert, in sonara-agent-activity-routes,
// clamps both fields. But `day_of_week` is nullable in the schema
// (`day_of_week is null or day_of_week between 0 and 6`), so the database
// permits a weekly row this would silently read as Sunday, and `isDue` is
// exported to three callers rather than reachable only from that insert.
//
// Three states, not two: a number, or not a number, and never a zero standing
// in for an answer nobody gave.
...
function isDue(schedule, now = new Date()) {
  ...
  const hour = wholeNumber(schedule.hour_of_day);
  if (hour === null || hour < 0 || hour > 23) return { due: false, reason: "The hour on this schedule is not a real hour." };
  ...
  if (cadence === "weekly") {
    const wanted = wholeNumber(schedule.day_of_week);
    if (wanted === null || wanted < 0 || wanted > 6) return { due: false, reason: "This weekly schedule does not name a day." };
    if (parts.weekday !== wanted) return { due: false, reason: "Not its day of the week." };
  }

  if (cadence === "monthly") {
    const wanted = wholeNumber(schedule.day_of_month);
    if (wanted === null || wanted < 1 || wanted > 28) return { due: false, reason: "This monthly schedule does not name a day of the month." };
...
module.exports = { CADENCES, isDue, describe, periodKey, localParts, knownZone, wholeNumber };
