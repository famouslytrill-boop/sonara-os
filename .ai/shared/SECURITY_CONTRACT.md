...
- Structured JSON and URL-encoded request bodies are capped at 1 MiB. Oversized structured bodies fail with HTTP `413` and `code=payload_too_large`; the server must not relabel the failure as HTTP 400.
- File bytes, base64 media, archives, audio, and video must not be proxied through general JSON routes or Vercel function bodies. Approved uploads use direct signed URLs to private storage, with file type, size, tenant path, ownership, and rights checks before acceptance.
...
- Provider connections fail closed until commercial terms, rate-limit handling, organization scope, server-only secrets, operator control, and optional-only AI rules pass deterministic validation.
- Purchase-order approval is role-separated: managers may submit; owners/admins approve or reject. The service-only RPC locks the organization-scoped row and writes audit evidence atomically.
- Location summaries consume consented stored events, reduce precision, and do not enable background tracking.
- Workflow planners reject unknown actions and never execute arbitrary code. Customer messaging, publishing, provider actions, and money-impacting work require human approval.
