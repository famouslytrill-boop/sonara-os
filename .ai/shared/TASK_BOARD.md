...
- [Codex] Repair the launch-readiness Preview UI: remove duplicated readiness aliases, show fail-closed Founder/Admin protection once, label the deployment environment, retain the machine-readable API contract, pass exact-head CI/Vercel preview, then merge and verify production.

## Blocked / owner-dependent

- [Owner + Codex] Configure an isolated non-production data backend for Preview testing, scope it to Preview or the specific branch, and redeploy. Do not reuse Production access across pull-request previews.
- [Owner, immediate] Replace previously disclosed production server access outside chat through approved private configuration surfaces.
...
- [Codex] PR #30 fixed payload-too-large request handling, passed application CI/dependency/Docker/database-preview/Vercel gates, merged as `af25aabd73a2df94a5c30bd157a8e1bbd1fc6c6f`, and deployed READY as `dpl_3S2YokV2p4Bn9UY7k1Xidp5PQG8f`.
...
- [Codex] PR #27 completed paid-launch application/database finalization; production database ledger remains 42/42 with linked schema lint passed.
