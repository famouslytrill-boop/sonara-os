...
export const openSourceTools: OpenSourceToolRecord[] = [
  ...
  },
  {
    ...
    license:
      "Sustainable Use License v1.0 -- source-available, NOT an open-source licence -- with source files containing \".ee.\" in the filename or \".ee\" in the dirname excluded from it entirely and requiring a paid n8n Enterprise License instead.",
    licenseRisk: "critical",
    reciprocalLicense: false,
    commercialUseStatus: "needs_review",
    integrationStatus: "needs_license_review",
    recommendedAction: [
      "no adapter until the licence question is answered by someone with authority to answer it",
      "complete license review",
      "self-host privately",
      "require approval for consequential workflows",
      "do not embed any of it in SONARA One or offer its functionality to customers -- that is not one of the three open questions below, it is forbidden outright, and only a commercial licence from n8n GmbH could change it",
      "do not read a non-master branch even for ideas: LICENSE.md says branches other than master are not licensed at all",
      "treat every \".ee\" file as unlicensed for this project regardless of anything else -- 1,197 were measured in the clone",
    ],
    officialUrl: "https://docs.n8n.io/",
    repoUrl: "https://github.com/n8n-io/n8n",
    notes: "Licence reviewed 12 August 2026 and deliberately left unresolved. The Sustainable Use License permits use, copying, distribution and derivative works for the licensee's own internal business purposes, and permits distribution to others only free of charge for non-commercial purposes. What it does not say is the part that matters here: it does not address hosting arrangements, it does not address a company self-hosting n8n for internal automations and calling it from separate software, and it does not address whether that changes when the calling product is commercial SaaS whose customers never touch n8n. Three questions, three silences. A reading that says this is plainly internal business use is available and may well be right, but it is a reading rather than something the text states, and this register does not record readings as findings. It stays out of the adapter set until somebody with authority to answer it does. That is a question for counsel, which is an owner decision and not a shipping step. ADDENDUM 10 September 2026: it arrived again as a mobile screenshot of the GitHub page with the request to add it to the application if possible, so the repository was shallow-cloned and LICENSE.md read directly for the first time. **That request is answerable and the answer is no**, and it is important not to confuse it with the three silences above: those are about SONARA self-hosting an instance and calling it from separate software, which remains a reading for counsel. Embedding n8n in SONARA One, or offering its functionality to SONARA customers, is a different arrangement and two independent clauses of the Limitations section each forbid it -- use is limited to \"your own internal business purposes\", and providing it to others is permitted \"only if you do so free of charge for non-commercial purposes\", while SONARA One is sold on plans from $29 to $1090 a month. Three restrictions found on this read that the August record did not carry, each a fact rather than a reading, and together the reason licenseRisk was raised from high to critical: files with \".ee.\" in the filename or \".ee\" in the dirname are explicitly NOT under the SUL and need an Enterprise key, and 1,197 of them were counted; \"Content of branches other than the main branch (i.e. master) are not licensed\", so a feature branch carries no grant whatsoever and even reading one for ideas has no licence behind it; and CONTRIBUTOR_LICENSE_AGREEMENT.md assigns contributions outright -- \"I give n8n permission to license my contributions on any terms they like\" -- so upstreaming is not a route to reusable rights. Measured 28,495 files excluding .git across 10 pnpm workspace packages at version 2.39.0, and its package.json declares no license field at all, which is why LICENSE.md rather than package metadata is the authority and why the \"Fair-code\" label on the repository page is not one. Two technical facts make it a poor fit even setting the licence aside: it declares engines node >=24.0.0 while this project runs Node 22 and declares >=22, and this application ships exactly one production dependency on a serverless runtime with a read-only filesystem and no long-lived process, whereas n8n is a stateful long-running server with its own queue and database. If the owner wants this capability in the product the routes are to build it or to buy a vendor that licenses it for resale; n8n Cloud is then a price rather than a licence question, and CLAUDE.md's rule that a hosted free tier is a price applies to it.",
    safetyBoundaries: ["scoped credentials", "human approval", "isolated encryption key", "audited jobs"],
    blockedUses: [
      "unsupervised outbound messages",
      "unapproved billing",
      "destructive workflows",
      "embedding, vendoring, forking or adapting any of its code into SONARA One",
      "offering its workflow automation to SONARA customers on any paid plan",
      "using any file with \".ee.\" in the name or \".ee\" in the path, which the SUL excludes entirely",
      "taking code or ideas from any branch other than master, which carries no licence",
    ],
    humanReviewRequired: true,
  },
  {
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    recommendedAction: [
      ...
      "measure the next one rather than assuming it -- a fourth arrived on 9 September 2026 (cporter202-best-apis-for-lead-gen) and the prediction was half right: same mega-list provenance and same p2hrc6 affiliate code, but no licence at all where these three are MIT, and an inline affiliate disclosure on 30 of its 31 pages where these three have none",
    ],
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    useCase: ["marketing frameworks an agent follows when working on SONARA copy and competitor comparisons"],
    productFit: ["Growth Studio", "Creator Studio"],
    license: "MIT",
    licenseRisk: "low",
    reciprocalLicense: false,
    ...
    integrationStatus: "adapter_built",
    recommendedAction: [
      "two skills adapted on 7 September 2026 -- .claude/skills/writing-sonara-marketing-copy and .claude/skills/comparing-sonara-to-a-competitor; adapt further ones the same way rather than vendoring the tree",
    ...
    notes:
      "Cloned and measured 7 September 2026. LICENSE read: MIT, Copyright (c) 2025 Corey Haines. 465 files, 335 markdown, 67 code files, and 50 SKILL.md files -- so it is a real skill library rather than a README with a directory listing. The structure the screenshot advertises is real: a product-marketing skill the others read first, then SEO, CRO, content, paid, retention and GTM branches. Two skills were adapted from it on 7 September 2026 -- writing-sonara-marketing-copy from its copywriting, copy-editing and product-marketing skills, and comparing-sonara-to-a-competitor from its competitors and competitor-profiling skills. Both were rewritten rather than copied: this library builds a `.agents/product-marketing.md` context document by interviewing the user about positioning, and SONARA's positioning is fixed in AGENTS.md with its market figures dated in docs/market/, so drafting a second undated statement of who we are would create exactly the uncheckable second copy this repository keeps deleting. scripts/verify-adapted-skills.mjs now ties this record to those two files in both directions.",
    safetyBoundaries: [
  ...
  },
  {
    ...
    integrationStatus: "adapter_built",
    recommendedAction: [
      "one skill adapted on 7 September 2026 -- .claude/skills/writing-a-social-post-for-sonara; adapt further ones the same way, with the MIT notice retained",
      "the follower-count framing was stripped rather than carried over -- audience numbers from somebody else's account are not evidence about ours",
    ],
    officialUrl: "https://github.com/charlie947/social-media-skills",
    repoUrl: "https://github.com/charlie947/social-media-skills",
    notes:
      "Cloned and measured 7 September 2026. LICENSE read: MIT, Copyright (c) 2026 Charlie Hills. 27 files, 22 markdown, 17 SKILL.md, one shell script. Small and readable. Its framing is one person's audience results, which is a reason to take the frameworks and leave the claims: repeating '350k followers' inside SONARA copy would be somebody else's proof presented as ours, and AGENTS.md puts proof publishing behind owner approval. Adapted on 7 September 2026 into .claude/skills/writing-a-social-post-for-sonara, with two things deliberately left behind. Its post-scorer pulls the author's real post history through Apify to score drafts against what performed, which is a better rubric than a static one and is not available here -- Apify is a hosted paid service in no record in this register, and CLAUDE.md is explicit that a free tier is a price rather than a licence. And its hook formula requires a 'How I' statement and a digit in every hook on clickbait principles, which is right for a personal brand and wrong for a company whose own rule is plain customer-facing language; manufacturing a digit is also how a post acquires a number nothing can support.",
    safetyBoundaries: [
  ...
  {
    name: "Best APIs for Lead Gen (cporter202)",
    slug: "cporter202-best-apis-for-lead-gen",
    category: ["API directory", "affiliate link list", "no licence declared", "all rights reserved"],
    useCase: ["none -- there is no grant to use it, and most of what it lists is blocked on consent grounds anyway"],
    ...
    integrationStatus: "blocked",
    recommendedAction: [
      "do not copy any part of it -- no LICENSE file means all rights reserved, and this is the first cporter202 API directory with no licence at all",
      "correct the standing prediction rather than citing it: the three earlier directories were MIT and disclosed nothing, this one is unlicensed and discloses every affiliate link",
      "do not adopt the listed products either -- 21 of its 31 API pages are scrapers or email finders, which AGENTS.md rules out on consent grounds independently of the licence",
    ],
    officialUrl: "https://github.com/cporter202/best-apis-for-lead-gen",
    repoUrl: "https://github.com/cporter202/best-apis-for-lead-gen",
    notes:
      "Cloned and measured 9 September 2026. No LICENSE, LICENCE or COPYING file anywhere in the tree, and no licence section in the README; both checked rather than inferred from the file listing. 79 files: 78 markdown and one PNG, and no executable code at all, so there is no dependency to take even if there were a grant. 46 links, 30 of them carrying `?fpr=p2hrc6` -- the same affiliate code recorded against the ecommerce and real-estate directories above. resources/source-audit.md says the provenance outright rather than leaving it to be measured: it records that the repository started as a broad API mega-list and that the curation deliberately kept `Existing affiliate tracking where present, especially ?fpr=p2hrc6`. Two things separate it from the three synced directories and both were measured, not assumed: there is no sync workflow or megaListSource config, and 30 of the 31 API pages carry an inline `Affiliate link: Yes` line naming the code. That is real disclosure, and it is why the reason this is blocked is the missing licence rather than concealment.",
    safetyBoundaries: [
      "no content copied from a repository with no licence grant",
      "no affiliate-tagged link reproduced on any SONARA surface, disclosed or not",
      "no bulk contact scraper or email finder adopted, which is a consent rule and not a licence one",
    ],
    blockedUses: ["copying, adapting or republishing any part of it", "adopting the scraper and email-finder products it lists"],
    humanReviewRequired: true,
  },
  {
    name: "Stock Market Signal Automation (cporter202)",
    slug: "cporter202-stock-market-signal-automation",
    category: ["webhook starter kit", "trading automation", "vendor funnel", "permissive licence"],
    useCase: ["nothing shipped -- the licence permits use and this product has no trading surface to use it on"],
    productFit: ["Internal Development"],
    license: "MIT",
    licenseRisk: "low",
    reciprocalLicense: false,
    ...
    integrationStatus: "reference_only",
    recommendedAction: [
      "read the webhook-signature verification example if a signed-webhook receiver is ever built here, and write our own",
      "do not build a trading or market-signal feature on it -- SONARA sells business, creator and growth operations, and financial signals are a regulated surface this project has no place in",
      "note that live signal data is a paid KamdenAI membership, so the repository is a funnel with sample payloads rather than a working data source",
    ],
    officialUrl: "https://github.com/cporter202/stock-market-signal-automation",
    repoUrl: "https://github.com/cporter202/stock-market-signal-automation",
    notes:
      "Cloned and measured 9 September 2026. LICENSE read: MIT, Copyright (c) 2026 Chris Porter. Recorded separately from the same author's API directories because measuring it said it is a different artefact, the way vibe-coding-with-base44 was: 49 files with real code in them -- six runnable examples across five JavaScript files and one Python file -- 14 links, and zero `?fpr=` affiliate parameters anywhere in the tree. It also ships DISCLAIMER.md, which states plainly that nothing in it is financial or trading advice and that past performance and backtests guarantee nothing. The commercial shape is still worth recording: the README's hero image is the KamdenAI homepage and it says live signal access comes through KamdenAI memberships, so the repository is sample payloads pointing at a paid service. That is a price rather than a licence, and CLAUDE.md treats those as constraints of the same weight. The reason not to use this is fit, not licence -- MIT genuinely permits it, and there is nothing here SONARA sells.",
    safetyBoundaries: [
      "no trading, investment or market-signal feature built into any SONARA product",
      "no dependency taken on a vendor membership for a shipped feature",
    ],
    humanReviewRequired: false,
  },
  {
    name: "BrowserCode (browser-use)",
    slug: "browser-use-browsercode",
    category: ["coding agent", "browser automation", "opencode fork", "permissive licence"],
    useCase: ["a developer-side tool at most, and not before somebody reviews what it is allowed to drive"],
    productFit: ["Internal Development"],
    license: "MIT (upstream opencode copyright, retained by the fork)",
    licenseRisk: "low",
    reciprocalLicense: false,
    commercialUseStatus: "allowed_after_review",
    integrationStatus: "needs_security_review",
    recommendedAction: [
      "never run it against a browser profile holding SONARA credentials -- it executes agent-written JavaScript in a live Chrome through unconstrained CDP, which is arbitrary code execution with whatever that profile is signed into",
      "do not install via the advertised `curl -fsSL https://bcode.sh/install | bash` -- piping a remote script into a shell runs code nobody read, on the machine that has this repository on it",
      "set DO_NOT_TRACK=1 before any trial: telemetry is on by default, which is the opposite of the default this project holds itself to",
    ],
    officialUrl: "https://github.com/browser-use/browsercode",
    repoUrl: "https://github.com/browser-use/browsercode",
    notes:
      "Cloned and measured 9 September 2026. LICENSE read: MIT -- and the copyright line is `Copyright (c) 2025 opencode`, not browser-use. That is not a defect: the README's own Architecture section states that BrowserCode is a fork of anomalyco/opencode with a vendored TypeScript port of browser-use/browser-harness, and retaining upstream copyright is what MIT requires of a fork. It still ships a packages/opencode directory. Worth recording because opencode arrived here as a separate recommendation on the same day, and the two are one codebase family rather than two finds: 6,255 files here against 6,560 upstream. The README also disclaims affiliation with the OpenCode team. Three properties matter more than the licence. It drives real Chrome through unconstrained CDP and the agent writes the JavaScript, so the trust boundary is whatever that browser is signed into. Its advertised install is a remote script piped to bash. And packages/bcode-browser/src/telemetry.ts sends anonymous traces unless DO_NOT_TRACK is set to a non-empty value, which is opt-out rather than opt-in.",
    safetyBoundaries: [
      "no browser profile carrying SONARA, Stripe, Supabase or Vercel credentials driven by an agent-written script",
      "no remote install script piped into a shell",
      "telemetry explicitly disabled before any trial",
    ],
    humanReviewRequired: true,
  },
  {
    name: "opencode (anomalyco)",
    slug: "anomalyco-opencode",
    category: ["coding agent", "developer tooling", "permissive licence"],
    useCase: ["a developer-side coding agent, read for how it is built rather than adopted into the product"],
    productFit: ["Internal Development"],
    license: "MIT",
    licenseRisk: "low",
    reciprocalLicense: false,
    ...
    integrationStatus: "reference_only",
    recommendedAction: [
      "treat it as a tool a developer may choose, never as something SONARA One depends on or ships",
      "read it alongside the BrowserCode record above -- BrowserCode is a fork of this, and the two arrived as separate recommendations hours apart",
      "any coding agent given repository access here inherits this repository's rules, including that service-role secrets stay server-only",
    ],
    officialUrl: "https://github.com/anomalyco/opencode",
    repoUrl: "https://github.com/anomalyco/opencode",
    notes:
      "Cloned and measured 9 September 2026. LICENSE read: MIT, Copyright (c) 2025 opencode. 6,560 files, overwhelmingly TypeScript (2,532 .ts plus 596 .tsx), a monorepo of packages with a desktop app, a server and a UI. No AGPL or GPL declaration in any package.json, checked rather than assumed from the top-level licence. There is nothing to integrate: SONARA One has one production dependency and no bundler, and a coding agent is not a product feature. The reason it is in the register is that it is the upstream of BrowserCode, which was recommended separately, and a register that records the fork without the thing it forked leaves the next reader to rediscover the relationship.",
    safetyBoundaries: [
      "no coding agent added as a runtime dependency of SONARA One",
      "no service-role key placed anywhere an agent tool can read it",
    ],
    humanReviewRequired: false,
  },
  {
    name: "Anthropic Agent Skills (anthropics/skills)",
    slug: "anthropics-skills",
    category: ["agent skills", "reference material", "no licence declared", "all rights reserved"],
    useCase: ["reading it is all that is available -- there is no grant to copy any of it into what this product ships"],
    productFit: ["Internal Development"],
    license: "None declared (all rights reserved)",
    licenseRisk: "critical",
    reciprocalLicense: false,
    ...
    integrationStatus: "blocked",
    recommendedAction: [
      "do not copy a skill from it into .claude/skills/ -- absence of a licence is not permission, and this project cannot grant what the author has not",
      "using these skills through Claude Code is a different thing entirely, and is governed by Anthropic product terms rather than by this repository",
      "only Anthropic adding a licence changes this, so read `blocked_until_review` as blocked",
    ],
    officialUrl: "https://github.com/anthropics/skills",
    repoUrl: "https://github.com/anthropics/skills",
    notes:
      "Cloned and measured 9 September 2026, and the finding is the one nobody expects: there is no licence. No LICENSE, LICENCE or COPYING file at any level, no licence section in the README, and no `license` field in any package.json -- all three checked, because the post recommending it called it an open source collection and a badge reading 3.1M invites the assumption. The only licence texts in the 419-file tree are OFL font licences bundled under skills/canvas-design/canvas-fonts/ and THIRD_PARTY_NOTICES.md, which attributes components in the shipped product and includes BSD-2-Clause and GPL-3.0 sections; neither is a grant covering this repository's own content. This project's own rule decides it: a repository with no licence declared is all rights reserved. The distinction that matters in practice, and that is easy to blur: these same skills are delivered to this session by Claude Code under Anthropic product terms, which is use of a product. Copying the repository's text into a skill SONARA ships is use of a work, and nothing here grants it.",
    safetyBoundaries: [
      "no skill text copied from a repository with no licence grant",
      "no claim in SONARA material that this repository is open source",
    ],
    blockedUses: ["copying or adapting any skill, spec or template into this repository"],
    humanReviewRequired: true,
  },
  {
    name: "Image Pipes (mrajaeim)",
    slug: "mrajaeim-image-pipes",
    category: ["computer vision", "visual pipeline editor", "self-hosted", "permissive licence"],
    useCase: ["nothing shipped -- a self-hosted desktop tool with no surface in this product that needs it"],
    ...
    integrationStatus: "reference_only",
    recommendedAction: [
      "do not plan an adapter against a local install -- it binds 127.0.0.1:8000 and a Vercel function cannot reach the owner's machine, which is the whole subject of docs/architecture/EXTERNAL-SERVICES.md",
      "if image processing is ever wanted in Creator Studio, revisit this as an owner-hosted service behind the four adapter rules, off by default like the other ten",
      "keep the albumentations finding attached to this record -- the pin is what makes it safe, and a later bump could change that",
    ],
    officialUrl: "https://github.com/mrajaeim/image-pipes",
    repoUrl: "https://github.com/mrajaeim/image-pipes",
    notes:
      "Cloned and measured 9 September 2026. LICENSE read: MIT, Copyright (c) 2026 Image Pipes Contributors. 220 files with real code -- 89 Python, 35 .tsx, 28 .ts -- a FastAPI backend, a React frontend, an Electron desktop shell, a Dockerfile and a compose file. The dependency licences were checked rather than taken from the badges, and one of them nearly went the other way: the README advertises Albumentations, and AlbumentationsX is dual-licensed AGPL-3.0 for open-source use, which triggers on network use and would oblige releasing this product's source. backend/pyproject.toml pins `albumentations==2.0.8`, the MIT package, not albumentationsx. opencv-python-headless is Apache-2.0. So there is no reciprocal exposure, and the reason is the pin rather than the family name. The practical limit is architectural: docker-compose binds it to 127.0.0.1:8000, and a serverless function cannot see a service on somebody's laptop.",
    safetyBoundaries: [
      "no adapter pointed at a loopback address, which the shared adapter base already refuses on serverless",
  ...
  {
    name: "DwarfStar (ds4-metal)",
    slug: "dwarfstar-ds4-metal-local-inference-engine",
    category: ["local model inference", "native inference engine", "Metal and CUDA and ROCm backends", "permissive licence"],
    useCase: [
      "nothing shipped -- a serverless function has no GPU, so this could only ever be a service the owner runs and the application calls",
    ],
    productFit: ["Creator Studio", "Internal Development"],
    license: "MIT. LICENSE read 11 September 2026: 'MIT License, Copyright (c) 2026 The ds4.c authors, Copyright (c) 2023-2026 The ggml authors'. A full Apache License 2.0 text is also bundled at licenses/Apache-2.0.txt, so the tree carries Apache-2.0 material alongside the MIT grant. Both are permissive and compatible.",
    licenseRisk: "low",
    reciprocalLicense: false,
    commercialUseStatus: "allowed_after_review",
    integrationStatus: "reference_only",
    recommendedAction: [
      "do not add as a dependency -- this application ships one production dependency and deploys to serverless functions with no GPU and no persistent disk",
      "if it is ever wanted, it is an adapter behind a URL variable exactly like Ollama, under the four rules in docs/architecture/EXTERNAL-SERVICES.md, and a serverless function cannot reach the owner's laptop",
      "price the hardware before the software: its own README names 96 GB Macs as the primary target and 128 GB for the larger models, which is a capital purchase rather than a per-token bill",
      "note the ggml copyright line when attributing -- the MIT grant covers two sets of authors, not one",
    ],
    officialUrl: "https://github.com/ivanfioravanti/ds4-metal",
    repoUrl: "https://github.com/ivanfioravanti/ds4-metal",
    notes:
      "Arrived 11 September 2026 as a social-media screenshot headlined 'Open source: ivanfioravanti/ds4-metal' with the README rendered as 'DwarfStar'. Shallow-cloned and read the same day. Measured 2,202 files excluding .git -- 1,256 .txt, 638 .json, 60 .c, 36 .py, 36 .md, 35 .cuh -- so the bulk is data and fixtures around a small C core rather than a large application. The README is unusually candid about scope: 'The code is self-contained and deliberately narrow, not a general GGUF runner: you need to use the GGUF files the project produces, that are part of the project itself', and 'This project would not exist without llama.cpp and GGML'. Supported hardware, quoted: Metal 'on Macs with 96 GB or more', NVIDIA CUDA with 'the DGX Spark is our main gaol' (sic), and ROCm on Strix Halo. The licence permits anything this project would want to do. The constraint is architectural and absolute rather than legal: SONARA One runs as Vercel serverless functions, which have no GPU, no persistent disk and a documented 300-second lifetime, so nothing here can run inside the application. Its only possible shape is the Open Media Worker pattern already described in docs/architecture/EXTERNAL-SERVICES.md -- the owner runs it, the application speaks one contract to it over a URL -- which makes the real cost a machine somebody buys and maintains.",
    safetyBoundaries: [
      "development-time or owner-hosted only: no inference engine can run inside a serverless function",
      "if ever adapted, it obeys the EXTERNAL-SERVICES rules -- off by default, never a dependency, never render its configuration, validate anything that becomes part of a request",
      "attribution covers both the ds4.c authors and the ggml authors",
    ],
    humanReviewRequired: true,
  },
  {
    name: "Ballast",
    slug: "ballast-kubernetes-workload-rightsizing-operator",
    category: ["Kubernetes operator", "resource right-sizing", "cluster cost control", "permissive licence"],
    useCase: [
      "nothing -- recorded because it was submitted, and because this project runs no Kubernetes at all",
    ],
    productFit: ["Internal Development"],
    license: "MIT. LICENSE read 11 September 2026: 'MIT License, Copyright (c) 2026 Tight Line LLC'.",
    licenseRisk: "low",
    reciprocalLicense: false,
    commercialUseStatus: "allowed_after_review",
    integrationStatus: "reference_only",
    recommendedAction: [
      "adopt nothing: there is no Kubernetes cluster in this project to install an operator into, and the licence is not what rules it out",
      "do not let a permissive licence read as applicability -- MIT says you may use it, not that there is anywhere to put it",
      "the transferable idea is the measurement, not the code: right-size from observed usage rather than from what somebody reserved, which is the same argument as sourcing a cost floor from real vendor prices",
    ],
    officialUrl: "https://github.com/tight-line/ballast",
    repoUrl: "https://github.com/tight-line/ballast",
    notes:
      "Arrived 11 September 2026 as a social-media screenshot showing Snyk, quality-gate and 100% codecov badges. Shallow-cloned and read the same day. Measured 186 files excluding .git -- 69 .go, 69 .yaml, 16 .md, 10 .yml -- a Go operator with its manifests. Its README states the mechanism plainly: 'Ballast is a Kubernetes operator that automatically right-sizes workload resource requests and limits based on real operational history. It is a more active alternative to Fairwinds Goldilocks: rather than suggesting changes, it applies them -- at admission time and on running pods via in-place resize (Kubernetes 1.35+).' The licence is clean and the engineering looks careful; the finding is that it has nothing to attach to here. SONARA One is Express on Vercel serverless functions with one production dependency -- there is no cluster, no pod, no VPA object and no admission webhook. Recording it as reference_only rather than leaving it unrecorded, so the next person who meets it does not re-read the licence to reach the same conclusion. Worth noting for honesty about its own limits: the README documents a 'Redeploy gap' where deleting a workload garbage-collects the VPA checkpoints its history lives in, so a GitOps teardown loses the data and it cold-starts again.",
    safetyBoundaries: [
      "nothing to install: a Kubernetes operator has no meaning in a serverless deployment",
      "a permissive licence is not a reason to adopt something the architecture has no place for",
    ],
    humanReviewRequired: false,
  },
  {
    name: "ClawFlows",
    slug: "clawflows-openclaw-workflow-blueprints",
    category: ["agent workflow collection", "markdown blueprints", "licence stated only in a README", "third-party agent runtime"],
    useCase: [
      "nothing shipped -- read as prior art for how agent workflows get described, if at all",
    ],
    productFit: ["Internal Development"],
    license: "Unclear, and the ambiguity is the finding. The README carries a 'License' section whose entire content is the word 'MIT'. There is NO LICENSE file anywhere in the repository -- checked by name across the whole tree on 11 September 2026 -- so the MIT text itself is absent, no copyright holder is named, and the warranty disclaimer and permission grant that make up the licence are not present.",
    licenseRisk: "medium",
    reciprocalLicense: false,
    commercialUseStatus: "needs_review",
    integrationStatus: "needs_license_review",
    recommendedAction: [
      "do not treat 'MIT' in a README as equivalent to an MIT licence file -- the grant, the copyright holder and the disclaimer are all missing, and those are the licence",
      "if anything here were ever wanted, ask the author to add a LICENSE file naming the holder; that is a one-file request and it resolves this entirely",
      "note there is nothing executable to take regardless: 141 of 179 files are markdown",
      "it targets OpenClaw rather than this stack, so even a clean licence would leave it as reading material",
    ],
    officialUrl: "https://github.com/nikilster/clawflows",
    repoUrl: "https://github.com/nikilster/clawflows",
    notes:
      "Arrived 11 September 2026 as a social-media screenshot describing it as 'a powerful collection of reusable workflow automation blueprints for OpenClaw'. Shallow-cloned and read the same day. Measured 179 files excluding .git -- 141 .md, 25 .bats, 2 .sh -- so it is a markdown collection with bats shell tests and essentially no application code, the same shape as the twelve-starter repository that turned out to be 57 markdown files and nothing executable. That makes it reference-only as a fact about the contents rather than as a caution. The licence position is the part worth recording carefully, because it sits between the two cases this register usually sees: it is not a declared licence, and it is not silence either. A README line reading 'MIT' is an expression of intent by the author, but the MIT licence is a specific text with a named copyright holder and a warranty disclaimer, and none of that is in the repository -- so there is nothing to comply WITH even if one wanted to. Recorded as needs_license_review rather than blocked, because the distinction between 'the author said MIT informally' and 'the author said nothing' is real and flattening it would overstate the finding.",
    safetyBoundaries: [
      "no code or content copied out of it until a LICENSE file exists",
      "a README word is not a licence text, and this register does not treat it as one",
    ],
    humanReviewRequired: true,
  },
  {
    name: "openclaw-api-list",
    slug: "openclaw-api-list-affiliate-link-directory",
    category: ["advertised as a curated API directory", "measured as an affiliate placement list", "no licence declared", "blocked"],
    useCase: [
      "none -- recorded so the measurement is on file and nobody curates from it by mistake",
    ],
    productFit: ["Internal Development"],
    license: "None declared. No LICENSE file and no licence statement in the README, checked 11 September 2026. Under the rule in CLAUDE.md, the absence of a licence is not permission: all rights reserved.",
    licenseRisk: "critical",
    reciprocalLicense: false,
    commercialUseStatus: "blocked_until_review",
    integrationStatus: "blocked",
    recommendedAction: [
      "do not use, copy, or cite this as a curated API directory -- 99.1% of its links are affiliate placements, which is a different kind of artefact than the label claims",
      "never surface any of these links to a customer: passing an undisclosed affiliate link to somebody who trusts the product is a disclosure problem before it is a licence problem",
      "no internal review can unblock it -- there is no licence to review, and only the author can grant one",
      "do not read the folder names as measurements: every one of the nineteen category folders contains exactly one file",
    ],
    officialUrl: "https://github.com/cporter202/openclaw-api-list",
    repoUrl: "https://github.com/cporter202/openclaw-api-list",
    notes:
      "Arrived 11 September 2026 as a social-media screenshot posted to a beginners' group, described as 'a ready to use API toolbox for experimenting with OpenClaw bots'. The GitHub About panel read 'This GitHub repo is a list of APIs you can plug into your OpenClaw bot to make it run like a powerhouse!' with 231 stars and 72 forks. Shallow-cloned and measured the same day, and the measurement is the record: of 78,913 links in the markdown, 78,216 carry an affiliate parameter -- 99.1% -- and every single one uses the same code, ?fpr=p2hrc6. Only 25 files exist in the whole repository excluding .git. The nineteen category directories are named with numbers that read as counts -- automation-apis-4825, lead-generation-apis-3452, social-media-apis-3268, developer-tools-apis-2652, ai-apis-1208 -- and each contains exactly one file; the numbers count nothing. A FOLLOW_CREATOR.md sits at the root. This is the third and fourth instance of this exact pattern in this register: the reviewing-an-outside-repository skill already records two 'curated API directories' that were affiliate placement lists at 2,273 of 2,283 and 1,090 of 1,096 links, both synced from one upstream. This one and cporter202/software-income-playbooks carry the identical affiliate code, which is direct evidence of the same arrangement rather than an inference.",
    safetyBoundaries: [
      "no affiliate link reaches a customer through this product, disclosed or otherwise",
      "no content copied out of a repository that grants no rights",
    ],
    blockedUses: [
      "using it as a source for any API list, catalogue, or integration directory in the product",
      "citing it as curated research in marketing copy or documentation",
      "copying any of its markdown, which is all rights reserved",
    ],
    humanReviewRequired: true,
  },
  {
    name: "software-income-playbooks",
    slug: "software-income-playbooks-affiliate-link-directory",
    category: ["advertised as curated playbooks and APIs", "measured as an affiliate placement list", "no licence declared", "blocked"],
    useCase: [
      "none -- recorded because it was submitted twice and the measurement should not have to be redone",
    ],
    productFit: ["Internal Development"],
    license: "None declared. No LICENSE file and no licence statement, checked 11 September 2026. All rights reserved.",
    licenseRisk: "critical",
    reciprocalLicense: false,
    commercialUseStatus: "blocked_until_review",
    integrationStatus: "blocked",
    recommendedAction: [
      "do not use it as a source of 'software ideas, curated APIs, and monetization paths' -- 99.1% of its links are affiliate placements under one code",
      "treat it and cporter202/openclaw-api-list as one artefact: they carry the identical affiliate code, so they are two labels on the same list",
      "no internal review can unblock it; only the author can grant a licence",
    ],
    officialUrl: "https://github.com/cporter202/software-income-playbooks",
    repoUrl: "https://github.com/cporter202/software-income-playbooks",
    notes:
      "Arrived 11 September 2026 as a social-media screenshot describing it as 'a practical library of software ideas, curated APIs, and monetization paths' and 'a great launchpad for turning useful data into real projects and products'. Shallow-cloned and measured the same day: of 78,884 links in the markdown, 78,186 carry an affiliate parameter -- 99.1% -- all of them the single code ?fpr=p2hrc6, which is byte-for-byte the code in cporter202/openclaw-api-list. Two repositories with different names, different framing and the same monetisation. 98 files excluding .git, across apis, playbooks, categories, resources, templates and archive. The honest description is an affiliate catalogue with a curation label, and the label is what makes it worth writing down: somebody searching this register for 'curated APIs' should find the measurement rather than the marketing.",
    safetyBoundaries: [
      "no affiliate link reaches a customer through this product",
      "no content copied out of a repository that grants no rights",
    ],
    blockedUses: [
      "sourcing product ideas, API lists or monetisation copy from it",
      "citing it as curated research anywhere customer-facing",
      "copying any of its markdown, which is all rights reserved",
    ],
    humanReviewRequired: true,
  },
  {
    name: "Vulture",
    slug: "vulture-osint-credential-harvesting-tool",
    category: ["OSINT automation", "breach-data lookup", "search-engine scraping", "no licence declared", "blocked on licence and on conduct"],
    useCase: [
      "none -- there is no version of this product that has a use for it, and that is the finding rather than a caution",
    ],
    productFit: ["Internal Development"],
    license: "None declared. No LICENSE file, checked 11 September 2026. All rights reserved.",
    licenseRisk: "critical",
    reciprocalLicense: false,
    commercialUseStatus: "blocked_until_review",
    integrationStatus: "blocked",
    recommendedAction: [
      "refuse on two independent grounds, either of which is sufficient: no licence grants any rights, and this product has no legitimate use for breach-data lookups or search-engine brute forcing",
      "do not read this as a position on authorised security testing -- it is that SONARA One is a platform for small businesses, and a reconnaissance tool has no place in it",
      "if credential-exposure checking is ever wanted as a customer FEATURE, that is a different thing built against a consented, contracted provider, not this",
    ],
    officialUrl: "https://github.com/vulture-osint-automation-tool/vulture",
    repoUrl: "https://github.com/vulture-osint-automation-tool/vulture",
    notes:
      "Arrived 11 September 2026 as a social-media screenshot tagged #pentesting #opensource with the source URL in the post. Shallow-cloned and read the same day. Seven files excluding .git: README.md, vulture.py, credharvest.py, passForge.py, api_keys.py, user_agents.py, requirements.txt. Its own README describes the modules: the Dehashed module 'will make a request to the Dehashed API which will return with data breaches associated with the target. This will include emails and passwords that have been leaked', requiring a paid Dehashed subscription with the key written into api_keys.py; the Google Dorking module runs selected dork categories and, in the README's own words, 'is a brute-force style program that will eventually alert Google bot detection. If detection is alerted, you must wait for a lockout cooldown or change IP.' Two refusals apply independently. The licence one is mechanical: no LICENSE file, so no rights are granted and none can be inferred. The conduct one does not depend on it: this is a reconnaissance tool for gathering leaked credentials about third parties and scraping a search engine in a way its author documents as triggering anti-abuse systems, and SONARA One sells business operations software to small businesses. There is no product surface here to attach it to and no internal review that would change that. Recorded rather than ignored so the refusal is written down once instead of re-argued.",
    safetyBoundaries: [
      "nothing from this enters the repository, the product, or a developer workflow here",
      "no third-party breach data is queried by or on behalf of this product",
      "no scraping that an operator's own documentation describes as triggering bot detection",
    ],
    blockedUses: [
      "any use, in any environment, under any configuration",
      "citing it as a security capability of this product",
    ],
    humanReviewRequired: true,
  },
  {
    name: "OpenContext",
    slug: "opencontext-agent-persistent-context-store",
    category: ["AI coding agent tooling", "persistent context store", "developer desktop tool", "three conflicting licence declarations"],
    useCase: [
      "possible development-time tool for whoever works on this codebase -- nothing customer-facing, and nothing until one licence is stated",
    ],
    productFit: ["Internal Development"],
    license: "Three different declarations in one repository, which is the finding. LICENSE is the MIT text, 'Copyright (c) 2025 OpenContext'. The root package.json declares 'license': 'Apache-2.0'. crates/opencontext-node/package.json declares 'ISC'. All three are permissive and none is reciprocal, so the practical risk is low -- but which one governs is unanswered, and the README badge renders whatever GitHub detects rather than what the author chose.",
    licenseRisk: "medium",
    reciprocalLicense: false,
    commercialUseStatus: "needs_review",
    integrationStatus: "needs_license_review",
    recommendedAction: [
      "ask the author to reconcile the three declarations before anything is copied out of it -- MIT, Apache-2.0 and ISC differ in attribution and patent terms even though all three permit commercial use",
      "trialling the published CLI as a local developer tool is a different question from copying its source, and the first is fine on any of the three readings",
      "keep it development-time: this is a context store for coding agents and has no place in the served application",
      "do not repeat the screenshot's 'license MIT' badge as the answer -- the badge is one of three declarations and the register should carry all three",
    ],
    officialUrl: "https://github.com/0xranx/OpenContext",
    repoUrl: "https://github.com/0xranx/OpenContext",
    notes:
      "Arrived 11 September 2026 as a social-media screenshot showing an 'MIT' licence badge and npm v0.2.2, described as capturing project knowledge 'so your coding agent remembers decisions and reuses context across repositories'. Shallow-cloned and read the same day; identified by search because the screenshot showed no owner, only three contributor avatars. Measured 242 files excluding .git. Published as @aicontextlab/cli, version 0.1.0 in the cloned root package.json against v0.2.2 on the badge, engines node >=18. The licence discrepancy was found by reading every package.json rather than trusting the badge: LICENSE says MIT, the root manifest says Apache-2.0, and the Rust node binding says ISC. That is the same principle this register already applies to duplicate records -- a repository with two verdicts has none -- turned on the licence itself. It is recorded as medium risk rather than critical because all three are permissive: the worst case is an attribution obligation somebody guessed at, not a prohibition. Its positioning is honest about cost, which is worth noting: 'Bring your own coding agent. OpenContext reuses your existing CLI (Codex/Claude/OpenCode) and adds a GUI + built-in Skills/tools -- no extra agent subscription', so it is not a metered service with a free tier that can be withdrawn.",
    safetyBoundaries: [
      "development-time only -- nothing from this reaches the served application or a customer",
      "no source copied until the three licence declarations are reconciled by the author",
      "a licence badge is a detection, not a declaration; the register records what the files say",
    ],
    humanReviewRequired: true,
  },
];
