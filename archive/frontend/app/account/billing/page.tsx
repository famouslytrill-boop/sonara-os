...
export default function BillingPage() {
  ...
  const safeEntitlementsByTier = entitlementsByTier ?? {};
    ...
    <ProductShell>
      <section className="rounded-lg border border-[#2A2A35] bg-[#171720] p-5 text-[#F8FAFC]">
        ...
        <p className="mt-3 max-w-3xl leading-7 text-[#A1A1AA]">
          Subscription state appears here after Supabase and Stripe webhook
          processing are configured. SONARA never shows fake active subscriptions.
        </p>
      ...
      <section className="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {safePricingTiers.map((tier) => (
          <article
            key={tier.id}
            className="rounded-lg border border-[#2A2A35] bg-[#171720] p-4"
          >
            ...
            <p className="mt-4 text-xs font-black uppercase text-[#22D3EE]">
              Entitlements
            </p>

            <ul className="mt-2 grid gap-1 text-sm leading-6 text-[#A1A1AA]">
              {(safeEntitlementsByTier[tier.id] ?? []).map((item) => (
                <li key={String(item)}>
      ...
      <Link
        className="mt-6 inline-flex min-h-11 items-center rounded-lg bg-[#8B5CF6] px-4 text-sm font-bold text-white"
        href="/pricing"
      >
        Review pricing
      </Link>
