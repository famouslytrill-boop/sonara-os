-- Connect the declared SONARA database contract to the server-side Supabase
-- runtime. This migration creates no autonomous agent executor and exposes no
-- customer data. It verifies existing schema/RLS state, makes service-role
-- object privileges explicit, and provides one service-only readiness RPC.
...
declare
  ...
  contract_tables constant text[] := array[
    ...
    'support_requests','feedback_reports','support_email_delivery_attempts',
    'service_catalog_items','service_requests','service_request_events','service_deliverables','service_comments','module_outputs','intake_requests','launch_checklist_items','activity_events',
    'business_workspaces','business_memberships','business_employee_invites','business_locations','inventory_items','vendor_accounts','vendor_invoices','recipe_cards','menu_items','customer_records','order_records',
    'creator_assets','creator_releases','creator_artist_systems','creator_song_blueprints','creator_prompt_packs','creator_release_packages',
    'growth_campaigns','growth_leads','growth_experiments',
    'provider_registry','feature_flags','workflow_runs','agent_action_logs','sonara_memory_records',
    'entities','entity_memberships','entity_agents','entity_agent_runs','entity_agent_memory','entity_agent_tool_registry','entity_proactive_actions','entity_action_runs','entity_action_approvals','entity_automations','entity_automation_runs','entity_connectors','entity_connector_events','platform_jobs',
    'audit_logs','admin_audit_logs','system_audit_events','db_health_snapshots','observability_events','license_reviews','security_reviews',
    'sonara_formula_groups','sonara_formula_definitions','sonara_formula_results'
  ];
begin
...
comment on function public.sonara_database_contract_snapshot() is
  'Service-role-only schema, RLS, and authorization-function readiness snapshot. Returns metadata only; never customer rows or secrets.';
