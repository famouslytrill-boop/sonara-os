...
do $
begin
  if to_regclass('public.business_work_orders') is null then
    raise notice 'skipping business_work_orders: table not present';
    return;
  end if;
  if not exists (
    select 1
    from information_schema.columns
    where table_schema = 'public'
      and table_name = 'business_work_orders'
      and column_name = 'organization_id'
  ) then
    raise notice 'skipping business_work_orders: required organization_id column not present';
    return;
  end if;

  execute 'alter table public.business_work_orders enable row level security';
  execute 'drop policy if exists "business_work_orders_select_member" on public.business_work_orders';
  execute 'create policy "business_work_orders_select_member" on public.business_work_orders for select to authenticated using (public.is_org_member(organization_id))';
end
$;
...
begin
  if to_regclass('public.purchase_orders') is null then
    raise notice 'skipping purchase_orders: table not present';
    return;
  end if;
  if not exists (
    select 1
    ...
    raise notice 'skipping purchase_orders: required organization_id column not present';
    return;
  end if;

  execute 'alter table public.purchase_orders enable row level security';
  execute 'drop policy if exists "purchase_orders_select_member" on public.purchase_orders';
  execute 'create policy "purchase_orders_select_member" on public.purchase_orders for select to authenticated using (public.is_org_member(organization_id))';
end
$$;
