...
1. User A creates a SoundOS organization.
2. User B creates a TableOS organization.
3. Confirm User A cannot select User B rows from tenant-owned tables.
4. Confirm User B cannot select SoundOS rows without `organization_app_access`.
...
8. Confirm AlertOS broadcast rows remain `pending_approval`.
