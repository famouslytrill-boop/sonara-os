...
// How much of the sprint log this document may carry, and why there is a limit
// at all.
//
// The first line of this file says "Paste this whole file as the first message
// to ChatGPT, Codex, or any other assistant picking up work on this
// repository." On 19 September 2026 that was measured: **1.25 MB, 23,536 lines,
// roughly 328,000 tokens**, of which 99.4% was the sprint log embedded
// verbatim. No ChatGPT tier accepts a 328k-token first message. The document
// could not do the one thing it opens by instructing you to do.
//
// That is the defect this repository is organised around, in the file whose
// entire job is to be the first thing somebody reads: a claim that is not true,
// stated confidently, at the top.
//
// So the history is bounded. The derived half -- the counts, the quoted safety
// rules, the approval categories, the real verify:launch chain -- is small
// (about 7 KB) and is what nobody can reconstruct without the repository, so it
// is always included in full. The sprint log is 1.24 MB of history that lives
// in `docs/SPRINT_LOG.md` and can be read there, so this carries the most
// recent entries and says how many it left behind.
//
// 128 KB is roughly 32,000 tokens: a first message that fits any current
// ChatGPT tier with the conversation still ahead of it. Measured 19 September
// 2026, that admits about the last twenty dated entries, which is a week of
// work at the current rate.
const HANDOFF_BUDGET_BYTES = 128 * 1024;

// Sliced on DATED headings only, so each included unit is a whole entry with
// its sub-sections intact. There are 393 `###` headings and 384 dated ones --
// the difference is sub-headings inside recent entries, and slicing on all of
...
function recentSprintEntries(log, budgetBytes) {
  ...
  const entries = starts.map((start, index) => log.slice(start, starts[index + 1] ?? log.length));
  const preamble = log.slice(0, starts[0]).trim();
  ...
  return { text: [preamble, ...kept].join("\n\n").trim(), included: kept.length, total: entries.length };
}
...
lines.push("");
lines.push("Paste this whole file as the first message to ChatGPT, Codex, or any other assistant picking up work on this repository. It is kept under " + (HANDOFF_BUDGET_BYTES / 1024) + " KB (roughly " + Math.round(HANDOFF_BUDGET_BYTES / 4 / 1000) + "k tokens) so that instruction is actually possible; `scripts/generate-handoff-prompt.mjs` fails the release if it grows past that.");
lines.push("");
...
if (sprintLog) {
  ...
  if (recent.total && !recent.included) {
    problems.push(
      `the newest sprint entry alone exceeds the remaining handoff budget (${budget} bytes). ` +
        "Either the entry is enormous or HANDOFF_BUDGET_BYTES is too small; this document must not ship with no history at all."
    );
  }
...
// The budget is asserted on the finished document, not hoped for.
//
// The slicing above bounds the sprint history, but the derived half can grow
// too -- more gates, more tables, more approval categories -- and a document
// that has quietly gone back over the limit is one that has quietly stopped
// being pastable. That is how it reached 1.25 MB the first time: nothing ever
// measured it.
//
// This runs in --check as well as on write, so `verify:handoff` in the release
// chain fails rather than a person noticing.
const generatedBytes = Buffer.byteLength(generated, "utf8");
if (generatedBytes > HANDOFF_BUDGET_BYTES) {
  problems.push(
    `the handoff prompt is ${(generatedBytes / 1024).toFixed(0)} KB, over the ${HANDOFF_BUDGET_BYTES / 1024} KB budget ` +
      `(roughly ${Math.round(generatedBytes / 4 / 1000)}k tokens against about ${Math.round(HANDOFF_BUDGET_BYTES / 4 / 1000)}k). ` +
      "Its first line tells the reader to paste the whole file into an assistant, so a document over the budget is a document " +
      "making a false promise. Either the sprint slice needs a smaller share or a section of the derived half has grown and should " +
      "point at the file it summarises instead of reproducing it."
  );
}
