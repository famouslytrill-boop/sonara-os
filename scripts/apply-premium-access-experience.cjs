...
const securityMiddleware = `app.use((req, res, next) => {
  ...
  res.setHeader("Content-Security-Policy", "default-src 'self'; base-uri 'self'; form-action 'self' https://checkout.stripe.com; frame-ancestors 'none'; object-src 'none'; img-src 'self' data: https:; style-src 'self' 'unsafe-inline'; script-src 'self'; connect-src 'self' https://*.supabase.co https://api.stripe.com; upgrade-insecure-requests");
  next();
});`;
...
source = source
  ...
  .replace('app.get("/account", (req, res) => {', 'app.get("/account", requireCustomer, (req, res) => {')
  .replace('app.get("/account/setup", (req, res) => {', 'app.get("/account/setup", requireCustomer, (req, res) => {')
...
console.log("SONARA premium access experience applied with protected account routes and public-copy cleanup.");
